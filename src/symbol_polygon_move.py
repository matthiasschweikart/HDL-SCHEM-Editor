...
class PolygonMove:
    def __init__(
        ...
        if create_binding:
            self.func_id_motion = self.diagram_tab.canvas.tag_bind(
                self.symbol.symbol_definition["object_tag"], "<Motion>", self.__move_polygon
            )
            self.func_id_button_release = self.diagram_tab.canvas.tag_bind(
                self.symbol.symbol_definition["object_tag"],
                "<ButtonRelease-1>",
    ...
    def __create_polygon_move_objects_for_other_polygons(self, event):
        polygon_move_list = (
            self.diagram_tab.get_polygon_move_list()
        )  # polygon_move_list = [[symbol_reference, canvas_id_selected], ...]
            ...
            if polygon_move_list_entry[1] != self.polygon_canvas_id:
                ...
                for port_list_entry in foreign_symbol.symbol_definition["port_list"]:
                    if port_list_entry["canvas_id"] == foreign_polygon_canvas_id:
                        ...
                        self.foreign_polygon_move_objects.append(
                            PolygonMove(
                                event,
                                self.window,
                                self.diagram_tab,
                                foreign_symbol,
                                foreign_polygon_canvas_id,
                                foreign_port_name_canvas_id,
                                False,
                            )
                        )
    ...
    def __get_information_from_connected_line(self, polygon_coords):
        ...
        for canvas_id in overlapping_ids:
            if self.diagram_tab.canvas.type(canvas_id) == "line" and "grid_line" not in self.diagram_tab.canvas.gettags(
                ...
                if (
                    abs(line_coords[0] - polygon_connection_point_x) < self.window.design.get_grid_size() / 10
                    and abs(line_coords[1] - polygon_connection_point_y) < self.window.design.get_grid_size() / 10
                ):
                    move_point = "first"
                    wire_ref = self.window.design.get_references([canvas_id])[0]
                elif (
                    abs(line_coords[-2] - polygon_connection_point_x) < self.window.design.get_grid_size() / 10
                    and abs(line_coords[-1] - polygon_connection_point_y) < self.window.design.get_grid_size() / 10
                ):
                    move_point = "last"
                    wire_ref = self.window.design.get_references([canvas_id])[0]
    ...
    def move_by_delta_or_by_sum(self, delta_x, delta_y, sum_in_rotation_direction, last_move):
        ...
        if (
            abs(polygon_rectangle_point_x - self.rectangle_coords[0]) < self.window.design.get_grid_size() / 10
            and abs(polygon_rectangle_point_y - polygon_connection_point_y) < self.window.design.get_grid_size() / 10
        ):  # left
            ...
            if polygon_rectangle_point_y_new >= self.rectangle_coords[3]:
                ...
                if sum_in_rotation_direction is None:
                    delta_x = (
                        0.5 * self.window.design.get_grid_size()
                    )  # Push polygon around the edge, so when is snaps to grid it always stays touching the rectangle.
                    sum_in_rotation_direction = delta_x + delta_y
                else:
                    delta_x = sum_in_rotation_direction - delta_y
                connection_point_delta_x, connection_point_delta_y = self.__rotate_polygon(
                    "positive", self.polygon_canvas_id, polygon_coords
                )
                connection_point_delta_x += (
                    0.5 * self.window.design.get_grid_size()
                )  # # Compensate distance from connection-point to rectangle.
                text_angle = "90"
                text_anchor = "w"
                ...
                if sum_in_rotation_direction is None:
                    ...
                    delta_x = -sum_in_rotation_direction + delta_y
                connection_point_delta_x, connection_point_delta_y = self.__rotate_polygon(
                    "negative", self.polygon_canvas_id, polygon_coords
                )
                connection_point_delta_x += (
                    0.5 * self.window.design.get_grid_size()
                )  # Compensate distance from connection-point to rectangle.
            ...
            abs(polygon_rectangle_point_x - self.rectangle_coords[2]) < self.window.design.get_grid_size() / 10
            and abs(polygon_rectangle_point_y - polygon_connection_point_y) < self.window.design.get_grid_size() / 10
        ):  # right
            if sum_in_rotation_direction is None:
                polygon_rectangle_point_y_new = polygon_rectangle_point_y + delta_y
            ...
            abs(polygon_rectangle_point_y - self.rectangle_coords[1]) < self.window.design.get_grid_size() / 10
            and abs(polygon_rectangle_point_x - polygon_connection_point_x) < self.window.design.get_grid_size() / 10
        ):  # top
            if sum_in_rotation_direction is None:
            ...
            abs(polygon_rectangle_point_y - self.rectangle_coords[3]) < self.window.design.get_grid_size() / 10
            and abs(polygon_rectangle_point_x - polygon_connection_point_x) < self.window.design.get_grid_size() / 10
        ):  # bottom
            if sum_in_rotation_direction is None:
                polygon_rectangle_point_x_new = polygon_rectangle_point_x + delta_x
    ...
    def __rotate_polygon(self, direction, polygon_canvas_id, polygon_coords):
        ...
        for point in range(4):
            diff_x, diff_y = (
                ...
                polygon_coords[2 * point + 1] - polygon_rectangle_point_y,
            )
            if direction == "negative":
                ...
                diff_x_rot = diff_y
                diff_y_rot = -diff_x
            polygon_coords_new[2 * point], polygon_coords_new[2 * point + 1] = (
                polygon_rectangle_point_x + diff_x_rot,
                polygon_rectangle_point_y + diff_y_rot,
            )
            if point == 0:
    ...
    def __move_polygon_end(self):
        ...
        self.diagram_tab.canvas.tag_unbind(
            self.symbol.symbol_definition["object_tag"], "<ButtonRelease-1>", self.func_id_button_release
        )
