...
class GenerateArchitecture:
    def __init__(
        ...
        instance_definitions = self.__get_instance_definitions(
            instance_connection_definitions, component_declarations_dict, generic_mapping_dict
        )
        ...
        self.architecture += self.__add_libraries_from_instance_configurations(
            libraries_from_instance_configuration, file_name
        )
        ...
        self.file_line_number += 1
        self.architecture += self.__add_schematic_elements(
            sorted_canvas_ids_for_hdl, hdl_dict, generate_dictionary, file_name
        )
    ...
    def __add_packages(self, file_name):
        ...
        if (
            text_dictionary["internals_packages"]
            and text_dictionary["internals_packages"] != ""
            and not text_dictionary["internals_packages"].isspace()
        ):
            ...
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "internals_packages", number_of_new_lines, "", ""
            )
    ...
    def __add_first_declarations(self, file_name):
        text_dictionary = self.design.get_text_dictionary()
        if (
            text_dictionary["architecture_first_declarations"]
            and text_dictionary["architecture_first_declarations"] != ""
            and not text_dictionary["architecture_first_declarations"].isspace()
        ):
            architecture_declarations = text_dictionary["architecture_first_declarations"]
            architecture_declarations = re.sub(
                "^([^\\n])", " " * 4 + "\\1", architecture_declarations, flags=re.MULTILINE
            )  # Add 4 blanks at the line start of a not empty line.
            if architecture_declarations[-1] != "\n":
                architecture_declarations += "\n"
            number_of_new_lines = architecture_declarations.count("\n")
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "first_declarations", number_of_new_lines, "", ""
            )
            self.file_line_number += number_of_new_lines
    ...
    def __add_signal_declarations(self, signal_declarations, file_name):
        ...
        for signal_declaration in signal_declarations:
            ...
            signal_name, _, _, _, _, _ = hdl_generate_functions.HdlGenerateFunctions.split_declaration(
                signal_declaration, "VHDL"
            )
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "signal_declaration", 1, signal_name, ""
            )
            self.file_line_number += 1
    ...
    def __add_last_declarations(self, file_name):
        text_dictionary = self.design.get_text_dictionary()
        if (
            text_dictionary["architecture_last_declarations"]
            and text_dictionary["architecture_last_declarations"] != ""
            and not text_dictionary["architecture_last_declarations"].isspace()
        ):
            ...
            number_of_new_lines = architecture_declarations.count("\n")
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "last_declarations", number_of_new_lines, "", ""
            )
            self.file_line_number += number_of_new_lines
            return architecture_declarations
        return ""
    ...
    def __add_schematic_elements(self, sorted_canvas_ids_for_hdl, hdl_dict, generate_dictionary, file_name):
        ...
        for canvas_id_for_hdl in sorted_canvas_ids_for_hdl:
            if (
            ...
            ):  # Filters symbols without connected wires (they have a canvas ID but are not part of hdl_dict).
                hdl_text = hdl_dict[canvas_id_for_hdl]["hdl_text"]
                hdl_type = hdl_dict[canvas_id_for_hdl]["type"]
                hdl_text = re.sub(
                    r"^\s*--\s*[0-9]+\s*\n", "", hdl_text
                )  # Remove the first line if it is a priority information comment line.
                if hdl_type == "generate":
                    generate_line_for_print = hdl_text
                    hdl_text = re.sub(
                        r"([^^]\s*--)\s*[0-9]+", r"\1", hdl_text
                    )  # Remove priority-comment, but leave rest of comment.
                    hdl_text = re.sub(r"([^^])\s*--\s*$", r"\1", hdl_text)  # Remove remaining empty comment.
                else:
                    hdl_text = re.sub(
                        r"([^^])\s*--\s*[0-9]+\s*$", r"\1", hdl_text, flags=re.MULTILINE
                    )  # Remove priority-comments at line end (from blocks or from instance-names).
            ...
            elif isinstance(canvas_id_for_hdl, str) and canvas_id_for_hdl.startswith(
                "begin-generate"
            ):  # The string "begin-generate ..." is an entry of sorted_canvas_ids_for_hdl.
                string_enclosed_canvas_ids = re.sub(r"begin-generate ", "", canvas_id_for_hdl)
                if not string_enclosed_canvas_ids:
                    messagebox.showwarning(
                        "HDl_SCHEM-Editor",
                        "There is an empty generate frame for this generate:\n"
                        + generate_line_for_print
                        + "\nSee file: "
                        + file_name,
                    )
                    ...
                    if type_of_canvas_id == "instance":
                        ...
                        if symbol_definition_of_canvas_id["configuration"]["config_statement"] == "Embedded":
                            ...
                            link_dictionary.LinkDictionary.link_dict_reference.add(
                                ...
                                "embedded_library_instruction",
                                1,
                                symbol_definition_of_canvas_id["instance_name"]["name"],
                                "",
                            )
                            self.file_line_number += 1
                if configuration != "":
                ...
                "end generate"
            ):  # The string "end generate ..." is an entry of sorted_canvas_ids_for_hdl.
                indent -= 4
    ...
    def __find_configurations_in_generates(self, sorted_canvas_ids_for_hdl):
        configurations = ""
        for canvas_id_for_hdl in sorted_canvas_ids_for_hdl:
            if isinstance(canvas_id_for_hdl, str) and canvas_id_for_hdl.startswith(
                "begin-generate"
            ):  # The string "begin-generate ..." is an entry of sorted_canvas_ids_for_hdl.
                string_enclosed_canvas_ids = re.sub(r"begin-generate ", "", canvas_id_for_hdl)
                list_of_enclosed_canvas_id_strings = string_enclosed_canvas_ids.split()
    ...
    def __fill_link_dictionary(self, hdl_text, file_name, canvas_id_for_hdl, hdl_type):
        ...
        if hdl_type == "generate":
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "generate", 1, canvas_id_for_hdl, ""
            )
            self.file_line_number += 1
        elif hdl_type == "block":
            for index in range(len(hdl_text_list)):
                link_dictionary.LinkDictionary.link_dict_reference.add(
                    self.design.window, file_name, self.file_line_number, "block", 1, canvas_id_for_hdl, index + 1
                )
                self.file_line_number += 1
        else:  # "symbol"
                ...
                if index == 0:
                    ...
                    if kind_of_line == "generic_mapping":
                        link_dictionary.LinkDictionary.link_dict_reference.add(
                            ...
                            1,
                            canvas_id_for_hdl,
                            local_line_number_in_generic_map,
                        )
                        local_line_number_in_generic_map += 1
        ...
        for entity_name, declarations in component_declarations_dict.items():
            ...
            if language != "VHDL":  # "Verilog", "SystemVerilog"
                ...
                for index, port_declaration in enumerate(port_declaration_list):
                    ...
                    port_declaration_list[index] = (
                        port_name + " : " + port_direction + port_type
                    )  # Attention: changes component_declarations_dict!
                # Add a type to the Verilog parameter definitions:
                component_declarations_dict[entity_name][1] = re.sub(
                    "=", ": integer :=", generic_definition
                )  # Only Verilog integer parameters are supported.
        ...
        for entity_name, declarations in component_declarations_dict.items():
            ...
            if insert_component:
                ...
                if port_declaration_list:
                    ...
                    port_declaration_list_indented = hdl_generate_functions.HdlGenerateFunctions.indent_identically(
    ...
    def __get_instance_definitions(
        self, instance_connection_definitions, component_declarations_dict, generic_mapping_dict
    ):
        unconnected_instance_dict, component_language_dict = self.__create_unconnected_instance_dict(
            component_declarations_dict
        )
        ...
        for (
            instance_name_def,
            generic_info,
        ) in generic_mapping_dict.items():  # instance_name_def contains the comment after the instance name
            if instance_name_def not in instance_connection_dict:
                instance_connection_dict[instance_name_def] = {
                    "entity_name": generic_info["entity_name"],
                    "canvas_id": generic_info["canvas_id"],
                    "connections": [],
                }
        ...
        for instance_name_def, instance_info in instance_connection_dict.items():
            instance_name = re.sub(
                r"\s*--.*", "", instance_name_def
            )  # Remove the priority information for order in HDL from the instance name.
            ...
            for connection in instance_info["connections"]:
                ...
                instance_declaration = re.sub(
                    r"( " + port_name + " +=> )open", "\\1" + connection[1] + connection[2], instance_declaration
                )
    ...
    def __create_unconnected_instance_dict(self, component_declarations_dict):
        ...
        for entity_name, declarations in component_declarations_dict.items():
            ...
            unconnected_instance_dict[entity_name] = self.__create_unconnected_instance_declaration(
                entity_name, component_port_declarations, generic_definition
            )
    ...
    def __create_instance_connection_dict(self, instance_connection_definitions, component_language_dict):
        ...
        for instance_connection_definition in instance_connection_definitions:
            ...
            signal_name, sub_range, _, _, _, signal_record_slice = (
                hdl_generate_functions.HdlGenerateFunctions.split_declaration(
                    signal_declaration, self.design.get_language()
                )
            )
            instance_connection_dict[instance_name]["entity_name"] = entity_name
            instance_connection_dict[instance_name]["connections"].append(
                [port_name, signal_name, sub_range + signal_record_slice, port_type]
            )
    ...
    def __create_unconnected_instance_declaration(self, entity_name, component_port_declarations, generic_definition):
        ...
        if generic_definition != "":
            open_instance += "\n" + " " * 4 + "generic map (\n#generic_definition#\n" + " " * 4 + ")"
        component_port_declarations = hdl_generate_functions.HdlGenerateFunctions.indent_identically(
            ":", component_port_declarations
        )
        if component_port_declarations:
            open_instance += "\n" + " " * 4 + "port map (\n"
            for declaration in component_port_declarations:
                port_connection_open = re.sub(
                    r":.*", r"=> open,\n", declaration
                )  # Leave the blanks between name and ':'.
