...
class CheckSensitivity:
    ...
    def __init__(
        ...
        list_of_processes, list_of_sensitivity_lists = self.__separate_sensitivity_lists_from_processes(
            list_of_processes, language
        )
        # print("check sensi started3: list_of_sensitivity_lists=", list_of_sensitivity_lists)
        self.__check_for_bug_in_sensitivity_list(
            list_of_processes,
            list_of_sensitivity_lists,
            list_of_readable_signals,
            language,
            self.module_name,
            hdl_file_name,
            hdl_code,
    ...
    def __get_list_of_processes(self, block_list, language):
        ...
        for canvas_id in block_list:
            ...
            list_of_processes.extend(
                self.__extract_processes(block_without_comments, language)
            )  # Each process has got a lot of additional blanks for easier searching.
    ...
    def __extract_processes(self, block_without_comments, language):
        ...
        block_without_comments = re.sub(
            r":", r" : ", block_without_comments
        )  # Separate labels (VHDL: from "process"; Verilog: from "begin")
        block_without_comments = re.sub(r"<", r" < ", block_without_comments)  # Surround with blanks.
        block_without_comments = re.sub(r">", r" > ", block_without_comments)  # Surround with blanks.
        block_without_comments = re.sub(r"=", r" = ", block_without_comments)  # Surround with blanks.
        block_without_comments = re.sub(r"\/", r" / ", block_without_comments)  # Surround with blanks.
        ...
        block_without_comments = re.sub(
            r"\)", r" ) ", block_without_comments
        )  # (VHDL: separate "process" from sensitivity; Verilog: separate conditions).
        block_without_comments = re.sub(r";", r" ; ", block_without_comments)  # Surround assignment end with blanks.
        block_without_comments = re.sub(r"\+", r" + ", block_without_comments)  # Surround '+' with blanks.
        block_without_comments = re.sub(r"-", r" - ", block_without_comments)  # Surround '-' with blanks.
        block_without_comments = re.sub(
            r"'[^\s]*?'", r"   ", block_without_comments
        )  # Remove any string (check for "not blank" is used to keep the "'" of attributes)
        ...
        if language == "VHDL":
            ...
            block_without_comments = re.sub(
                r"^process", r" process", block_without_comments, flags=re.IGNORECASE | re.MULTILINE
            )
            block_without_comments = re.sub(
                r"process$", r"process ", block_without_comments, flags=re.IGNORECASE | re.MULTILINE
            )
            # print("__extract_processes: block_without_comments =", block_without_comments)
            match_object_without_sensitivity_list = re.search(
                r"\s+process\s+?[^(\s]", block_without_comments, flags=re.DOTALL
            )  # Check the opening bracket of the sensitivity list
            match_object = re.search(
                r" process .*?endprocess", block_without_comments, flags=re.DOTALL
            )  # Separate each process
            while match_object:
                if (
                    not match_object_without_sensitivity_list
                    and "rising_edge" not in match_object.group(0)
                    and "falling_edge" not in match_object.group(0)
                    and "event" not in match_object.group(0)
                ):
                    process_list.append(match_object.group(0).split())
                # Remove the last hit:
                block_without_comments = re.sub(
                    r" process .*?endprocess", "", block_without_comments, count=1, flags=re.DOTALL
                )
            ...
            for word in list_of_words:
                if word == "always":
                    ...
                    if not wait_for_begin and begin_counter == 0:
                        if "posedge" not in process and "negedge" not in process:
    ...
    def __check_for_bug_in_sensitivity_list(
        self,
        list_of_processes,
        list_of_sensitivity_lists,
        list_of_readable_signals,
        ...
        for index, process in enumerate(list_of_processes):
            if (language == "VHDL" and "all" not in list_of_sensitivity_lists[index]) or (
                ...
                for readable_signal in list_of_readable_signals:
                    found_readable_signal_in_sensitivity_list = bool(
                        readable_signal in list_of_sensitivity_lists[index]
                    )
                    ...
                    if found_readable_signal_in_sensitivity_list != found_readable_signal_in_process:
                        self.__warn_user_because_of_problem_in_sensitivity(
                            list_of_sensitivity_lists[index],
                            hdl_code,
                            found_readable_signal_in_sensitivity_list,
                            module_name,
                            readable_signal,
                            hdl_file_name,
                        )

    def __warn_user_because_of_problem_in_sensitivity(
        self,
        sensitivity_list,
        hdl_code,
        found_readable_signal_in_sensitivity_list,
        ...
        if match_object is not None:
            ...
            if found_readable_signal_in_sensitivity_list is False:
                self.sensitivity_message += (
                    "HDL Sensitivity  : Warning in module "
                    + module_name
                    + ": The signal "
                    + readable_signal
                    + " is missing in the sensitivity-list "
                    + sensitivity_list
                    + " in line "
                    + str(line_number)
                    + " of file "
                ...
                self.sensitivity_message += (
                    ...
                    + ": The signal "
                    + readable_signal
                    + " is not needed in the sensitivity-list "
                    + sensitivity_list
                    + " in line "
                    + str(line_number)
                    + " of file "
