...
class GenerateModuleContent:
    ...
    def __init__(
        ...
        self.module_content += self.__add_schematic_elements(
    ...
    def __add_first_declarations(self, file_name):
        ...
        if (
            ...
            declarations = re.sub(
                "^([^\\n])", " " * 4 + "\\1", declarations, flags=re.MULTILINE
    ...
    def __get_instance_definitions(
        ...
        for instance_name_def, instance_info in instance_connection_dict.items():
            ...
            instance_declaration = re.sub(
                r"instance-name \(",
                instance_name_without_comment + " (" + comment_of_instance_name,
                unconnected_instance_dict[entity_name],
            )
            if component_language_dict[entity_name] == "VHDL":
                ...
                if instance_name_def not in generic_mapping_dict:
                    messagebox.showerror(
                        ...
                        "The instance name "
                        + instance_name_without_comment
                        + " is not found in database. HDL will be corrupted",
                    )
                    return {}
    ...
    def __add_schematic_elements(self, sorted_canvas_ids_for_hdl, hdl_dict, generate_dictionary, file_name):
        ...
        for canvas_id_for_hdl in sorted_canvas_ids_for_hdl:
            if (
            ...
            ):  # Filter symbols without connected wires (they have a canvas ID but are not part of hdl_dict).
                hdl_text = hdl_dict[canvas_id_for_hdl]["hdl_text"]
                hdl_text = re.sub(
                    r"^\s*//\s*[0-9]+\s*\n", "", hdl_text
                )  # Remove the first line if it is a comment line which has only priority information.
                hdl_text = re.sub(
                    r"([^^])\s*//\s*[0-9]+\s*$", r"\1", hdl_text, flags=re.MULTILINE
                )  # Remove priority-comments at line end (from generates or instance-names).
                ...
                "end generate"
            ):  # "end generate ..." is an entry of sorted_canvas_ids_for_hdl.
                indent -= 4
    ...
    def __fill_link_dictionary(self, hdl_text, file_name, canvas_id_for_hdl, hdl_type):
        ...
        if hdl_type == "generate":
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "generate", 2, canvas_id_for_hdl, ""
            )  # "generate" line and "begin" line
            ...
            for index, hdl_line in enumerate(hdl_text_list_stripped):
                if index == 0 and not hdl_line.endswith("#("):
                    link_dictionary.LinkDictionary.link_dict_reference.add(
                        self.design.window, file_name, self.file_line_number, "instance_name", 1, canvas_id_for_hdl, ""
                    )
                    self.file_line_number += 1
                    ...
                    if kind_of_line == "generic_mapping":
                        ...
                        signal_name = re.sub(
                            r".*\((.*)\).*", r"\1", hdl_line
                        )  # extract connected signal from parenthesis
    ...
    def __create_instance_connection_dict(self, instance_connection_definitions, component_language_dict):
        ...
        for instance_connection_definition in instance_connection_definitions:
            ...
            if (
                instance_name not in instance_connection_dict
            ):  # Only the first connection of an instance initializes the dictionary.
            ...
            signal_name, sub_range, _, _, _, _ = hdl_generate_functions.HdlGenerateFunctions.split_declaration(
                signal_declaration, self.design.get_language()
            )
            signal_name = re.sub(
                r"\[.*", "", signal_name
            )  # The array range of a signal must not be used, when the signal is connected to a port.
