...
from codegen import hdl_generate_functions, list_separation_check
...
class GenerateModuleInterface:
    def __init__(self, design, input_declarations, output_declarations, inout_declarations, file_name):
        ...
        self.module += self.__get_port_declarations(
    ...
    def __get_parameter_declarations(self, text_dictionary, file_name):
        ...
        if parameters != "" and not parameters.isspace():
            parameters = re.sub(
                "^([^\\n])", " " * 8 + "\\1", parameters, flags=re.MULTILINE
            )  # Add 8 blanks at the line start of not empty lines.
            if parameters[-1] != "\n":
                parameters += "\n"
            ...
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "interface_generics", number_of_new_lines, "", ""
            )
    ...
    def __get_port_declarations(self, input_declarations, output_declarations, inout_declarations, file_name):
        ...
        for declaration in declaration_list:
            ...
            link_dictionary.LinkDictionary.link_dict_reference.add(
                self.design.window, file_name, self.file_line_number, "port_declaration", 1, port_name, ""
            )
            self.file_line_number += 1
