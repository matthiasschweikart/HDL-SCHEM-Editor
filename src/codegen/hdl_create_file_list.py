...
class HdlCreateFileList:
    def __init__(self, parent, window, log_tab):
        ...
        success = self.__create_hdl_file_list_for_this_design(
            self.hdl_file_list, hdl_schem_editor_design_dictionary, top_library, active_architecture, module_name_list
        )
        if success:
    ...
    def __create_hdl_file_list_for_this_design(
        ...
        symbol_definitions = self.window.design.get_symbol_definitions_from_design_dictionary(
            hdl_schem_editor_design_dictionary
        )
        symbol_library = "_dummy_"
        for symbol_definition in symbol_definitions:
            if not symbol_definition["filename"].endswith(".hse"):
                symbol_library = self.__add_lib_entry_for_symbol(
                    hdl_file_list, symbol_definition, symbol_library, design_library
                )
        ...
        if additional_files_string != "":
            ...
            if additional_files_list:
                hdl_file_list.append(
                    "lib: " + design_library
                )  # neccessary because a hdl-file-list in additional files might have changed the library.
        if hdl_schem_editor_design_dictionary["language"] == "VHDL":
            if hdl_schem_editor_design_dictionary["number_of_files"] == 1:
                filename = (
                    hdl_schem_editor_design_dictionary["generate_path_value"]
                    + "/"
                    + hdl_schem_editor_design_dictionary["module_name"]
                    + ".vhd"
                )
    ...
    def __add_file_entries_for_symbol(
        self, hdl_file_list, symbol_definition, symbol_library, design_library, module_name_list
    ):
        ...
        if path_name.endswith(".hse"):
            try:
                ...
                if "active__architecture" in hdl_schem_editor_design_dictionary_sub:
                    hdl_schem_editor_design_dictionary_sub = (
                        self.__get_design_dict_from_dict_with_several_architectures(
                            symbol_definition["architecture_name"], hdl_schem_editor_design_dictionary_sub
                        )
                    )
                if "architecture_name" not in hdl_schem_editor_design_dictionary_sub:
                    active_architecture = (
                        "struct"  # Old versions of HDL-SCHEM-Editor do not store the architecture name.
                    )
                else:
                ...
                language = hdl_schem_editor_design_dictionary_sub["language"]
                if self.__hdl_is_not_up_to_date(
                    path_name, language, generate_path_name, module_name, number_of_files, active_architecture
                ):
                    return False  # Error message is generated by __hdl_is_not_up_to_date().
                symbol_library = self.__create_hdl_file_list_for_this_design(
                    hdl_file_list,
                    hdl_schem_editor_design_dictionary_sub,
                    design_library,
                    active_architecture,
                ...
                messagebox.showerror(
                    ...
                    "File " + symbol_definition["filename"] + " could not be found. hdl-file-list will be incomplete.",
                )
            ...
            for additional_source_file in symbol_definition["additional_files"]:
                if (
                    ...
                    and not additional_source_file.isspace()
                    and additional_source_file not in hdl_file_list
                ):
            ...
            symbol_definition["filename"].endswith(".vhd")
            or symbol_definition["filename"].endswith(".v")
            or symbol_definition["filename"].endswith(".sv")
        ):
            for additional_source_file in symbol_definition["additional_files"]:
                if (
                    additional_source_file != ""
            ...
            if (
                ...
                and symbol_definition["architecture_filename"] not in hdl_file_list
            ):
                hdl_file_list.append(symbol_definition["architecture_filename"])
        return symbol_library

    def __hdl_is_not_up_to_date(
        self, path_name, language, generate_path_name, module_name, number_of_files, architecture_name
    ):
        if language != "VHDL":
        ...
        return hdl_generate_functions.HdlGenerateFunctions.hdl_must_be_generated(
            path_name, hdlfilename, hdlfilename_architecture, show_message=True
        )

    def __get_design_dict_from_dict_with_several_architectures(
        ...
        if architecture_name in hdl_schem_editor_design_dictionary_sub:
            ...
            if architecture_name != "":
                messagebox.showerror(
                    "Error at creating the hdl-file-list",
                    "The data base of module "
                    + module_name
                    + " has several architectures (only supported for VHDL).\n"
                    + 'But the architecture "'
                    + architecture_name
                    + '", specified in the symbol properties of the module '
                    + hdl_schem_editor_design_dictionary_sub[active_architecture]["module_name"]
    ...
    def __save_in_file(self, hdl_file_list, log_tab, module_name):
        ...
        try:
            ...
            log_tab.log_frame_text.insert_line(
                "Created          : " + self.hdl_file_list_name + "\n", state_after_insert="disabled"
            )
        except FileNotFoundError:
