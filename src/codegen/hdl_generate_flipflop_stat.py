...
class GenerateFlipflopStat:
                ...
                if filename.endswith(".vhd") or filename.endswith(".vhdl"):  # external VHDL could use ".vhdl"
                    ...
                    if architecture_name != "":
                        ...
                        (
                            ...
                            signal_ranges,
                            signals_clocked,
                            signals_clocked_generate_condition_lists,
                        ) = self.__get_signals(vhdl_parser_object)
                        ...
                        dummy_type_declarations, report_commands = self.__create_report_commands(
                            entity_name_in_work,
                            signals_clocked,
                            signals_clocked_generate_condition_lists,
                            declared_names,
                        ...
                        self.__create_extended_vhdl_file(
                            filename_with_architecture, hdl, dummy_type_declarations, report_commands, architecture_name
                        )
                        filenames_to_be_changed.append(filename)
                        self.hdl_file_list_for_ff_stat = self.__modify_hdl_file_list(
                            hdl_file_list_name, filenames_to_be_changed
                        )
                else:
                    # Verilog file
                    verilog_parser_object = verilog_parsing.VerilogParser(hdl, "module", parse_big_files=False)
                    _, _, _, signals_clocked, signals_clocked_generate_condition_lists = self.__get_signals(
                        verilog_parser_object
                    )
                    report_commands = self.__create_verilog_report_commands(
                        signals_clocked, signals_clocked_generate_condition_lists
                    )
    ...
    def __create_report_commands(
        ...
        report_commands = []
        for signal_index, clocked_signal_name in enumerate(
            signals_clocked
        ):  # Check each signal which gets a value in a clocked process.
            for declared_name_index, declared_name in enumerate(declared_names):  # Search for the signal declaration.
                if declared_name == clocked_signal_name:
                    ...
                    if clocked_signal_generate_conditions:
                        self.__create_report_command_for_generate_condition_of_signal(
                            clocked_signal_generate_conditions,
                            report_commands,
                            entity_name_in_work,
                            clocked_signal_name,
                        )
                    ...
                    if clocked_signal_type in [
                        ...
                        self.__append_report_commands(
                            clocked_signal_type,
                            clocked_signal_name,
                            clocked_signal_range,
                            entity_name_in_work,
                            dummy_type_declarations,
                            report_commands,
                        )
                        ...
                        for type_declaration in type_declarations:
                            if clocked_signal_type == type_declaration[1]:  # In index 1 the type name is stored
                                ...
                                if "record" in type_declaration:
                                    self.__append_report_commands_from_record(
                                        type_declaration,
                                        clocked_signal_name,
                                        clocked_signal_range,
                                        entity_name_in_work,
                                    ...
                                    self.__append_report_command_from_user_type(
                                        entity_name_in_work,
                                        clocked_signal_name,
                                        clocked_signal_type,
                                        type_declaration,
                        ...
                        if not type_definition_found:
                            report_commands.append(
                                'report "flipflop_statistics for instance " & '
                                + entity_name_in_work
                                + "'path_name & "
                                + '" signal '
                                + clocked_signal_name
                                + " with type "
                                + clocked_signal_type
                                + ' needs an unknown number of flipflops (no type definition was found).";'
                            )
        if not report_commands:
            report_commands.append(
                ...
                + entity_name_in_work
                + "'path_name & "
                + '" no clocked signals found.";'
            )
        return dummy_type_declarations, report_commands

    def __append_report_commands_from_record(
        self,
        type_declaration,
        clocked_signal_name,
        clocked_signal_range,
        ...
        for record_slice_type_declaration in self.__split_list(type_declaration):
            ...
            self.__append_report_commands(
                clocked_record_slice_type,
                clocked_signal_name + "." + clocked_record_slice_name,
    ...
    def __append_report_commands(
        ...
        if clocked_signal_type in ["integer", "natural", "positive", "negative"]:
            dummy_type_declarations.append(
                "subtype t_ff_stat_"
                + str(self.sub_type_identifier)
                + " is "
                + clocked_signal_type
                + " "
                + clocked_signal_range
            ...
            report_commands.append(
                ...
                + clocked_signal_name
                + ' uses "'
                + " & integer'image(integer(ceil(log(real(abs(t_ff_stat_"
                + str(self.sub_type_identifier)
                + "'right "
                + " - "
                + "t_ff_stat_"
    ...
    def __create_report_command_for_generate_condition_of_signal(
        self, clocked_signal_generate_conditions, report_commands, entity_name_in_work, clocked_signal_name
    ):
        complete_condition = ""
    ...
    def __create_verilog_report_commands(self, signals_clocked, signals_clocked_generate_condition_lists):
        ...
        for index, clocked_signal_name in enumerate(signals_clocked):
            ...
            report_commands.append(
                '$display("flipflop_statistics for instance %m signal '
                + clocked_signal_name
                + ' generate-condition == ", '
                + generate_condition
                + ");"
    ...
    def __append_report_command_from_user_type(
        self,
        entity_name_in_work,
        clocked_signal_name,
        clocked_signal_type,
        ...
        if type_declaration_for_clocked_signal[3] == "(":  # enumeration type
            ...
            for word in type_declaration_for_clocked_signal:
                if word == ")":
                    ...
                    report_commands.append(
                        ...
                        + clocked_signal_name
                        + ' uses "'
                        + " & integer'image(integer(ceil(log(real("
                        + clocked_signal_type
                        + "'pos("
                        + rightmost_value
                        + ") + 1))/log(2.0)))) & "
            ...
            depth_of_type_definition, element_type_name, element_range_definition = (
                self.__determine_depth_of_array_type_definition(
                    clocked_signal_type, type_declarations, depth_of_type_definition
                )
            )
            if depth_of_type_definition == 0:
                report_commands.append(
                    ...
                    + " with type "
                    + clocked_signal_type
                    + ' needs an unknown number of flipflops.";'
                )
            else:
    ...
    def __create_extended_vhdl_file(
        self, filename_with_architecture, hdl, dummy_type_declarations, report_commands, architecture_name
    ):
