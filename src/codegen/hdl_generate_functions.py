...
from elements import symbol_instance
...
class HdlGenerateFunctions:
    ...
    def split_declaration(cls, signal_declaration, language):
        ...
        if language == "VHDL":
            ...
            if "." in signal_declaration:  # Then the signal is a slice of a record
                signal_record_slice = re.sub(
                    r".*\.", ".", signal_declaration
                )  # remove all characters until the '.' is reached.
                signal_record_slice = re.sub(
                    r"\s*:.*", "", signal_record_slice
                )  # remove all characters after the slice.
            else:
                signal_record_slice = ""
            ...
            if comment == signal_declaration:
                ...
                signal_declaration = re.sub(
                    r"\s*//.*", "", signal_declaration
                )  # Remove comment from signal_declaration.
            initialization = ""  # Verilog does not support initialization at declaration (only in initial blocks).
            signal_declaration = re.sub(
                r"\s*:\s*", ":", signal_declaration
            )  # Remove blanks from ranges and from index-separator at the end of line.
            ...
            signal_declaration = re.sub(
                r"(\[.*?):(.*?\])", r"\1|\2", signal_declaration
            )  # Substitute ':' in all range descriptions to be able to search for another ':'
            if (
                ":" in signal_declaration
            ):  # Then a subrange exists (this construct is not valid Verilog, but a extension here).
                signal_sub_range = re.sub(r".*:", "", signal_declaration)  # Remove all but signal_sub_range.
                signal_sub_range = re.sub(
                    r"\|", ":", signal_sub_range
                )  # Undo the replacement of ':' in signal_sub_range.
                signal_sub_range = re.sub(r" ", "", signal_sub_range)  # Remove blanks from signal_sub_range.
                signal_declaration = re.sub(
                    r"\s*:.*", "", signal_declaration
                )  # Remove sub_range from signal_declaration.
            else:
                signal_sub_range = ""
            signal_declaration = re.sub(
                r"\|", ":", signal_declaration
            )  # Undo the replacement of ':' in signal_declaration.
    ...
    def extract_data_from_symbols(cls, symbol_definition_list):
        ...
        for symbol_definition in symbol_definition_list:
            ...
            entity_name = symbol_instance.Symbol.get_entity_name(
                symbol_definition
            )  # Only needed for symbols with empty pin_definition_list.
            instance_name = symbol_instance.Symbol.get_instance_name(
                symbol_definition
            )  # Only needed for symbols with empty pin_definition_list.
            ...
            library_from_instance_configuration = symbol_instance.Symbol.get_library_from_instance_configuration(
                symbol_definition
            )
            ...
            generic_mapping_dict[instance_name] = {
                "entity_name": entity_name,
                "generic_map": generic_map,
                "canvas_id": symbol_instance.Symbol.get_canvas_id(symbol_definition),
            }
        ...
        if not os.path.isfile(path_name):
            ...
            if name_of_file.endswith(".hse"):
                messagebox.showerror(
                    "Error in HDL-SCHEM-Editor", "The HDL-SCHEM-Editor project file " + path_name + " is missing."
                )
            else:
                messagebox.showerror(
                    "Error in HDL-SCHEM-Editor", "The HDL-FSM-Editor project file " + path_name + " is missing."
                )
            return True
        ...
        if hdlfilename_architecture is not None:
            if not os.path.isfile(hdlfilename_architecture):
                if show_message:
                    messagebox.showerror(
                        "Error in HDL-SCHEM-Editor",
                        "The architecture file " + hdlfilename_architecture + " is missing.",
                    )
                return True
