...
class ExtractHierarchy:
    ...
    def __get_list_of_submodule_dicts_for_module(self, module_name):
        ...
        if (
        ...
        ):  # If only an entity (without architecture) is used for the instance, then "instance_types" does not exist.
            ...
            for index, sub_module_name in enumerate(instances_in_this_module):
                if (
                    sub_module_name != "process"  # Processes are instances without any hierarchy and are ignored.
                    and sub_module_name in self.used_modules_dict
                ):  # When additional files are missing, entries will be missing in self.used_modules_dict.
                    sub_module_dict = {}
                    if self.used_modules_dict[module_name]["target_library"][index] is not None:
                        sub_module_dict["configuration_library"] = self.used_modules_dict[module_name][
                            "target_library"
                        ][index]
                    else:
                    ...
                    sub_module_dict["architecture_filename"] = self.used_modules_dict[sub_module_name][
            ...
            try:
                ...
                if entity_name == "":
                    if entity_name_used_in_arch == "":
                        ...
                        if (
                            entity_name_used_in_arch not in used_modules_dict
                        ):  # Then the file with the entity was not read until now.
                    ...
                    if (
                        entity_name not in used_modules_dict
                    ):  # Then the file with the architecture was not read until now.
                ...
                if entity_name_used_in_arch != "":  # An architecture or a Verilog-file is read.
                    ...
                    if language == "VHDL":
                        used_modules_dict[entity_name_used_in_arch]["architecture_name"] = hdl_dict.get(
                            "architecture_name"
                        )
                    else:
                    ...
                    used_modules_dict[entity_name_used_in_arch]["target_library"] = [None] * len(
                        hdl_dict.get("label_names")
                    )
                    used_modules_dict[entity_name_used_in_arch]["target_module"] = [None] * len(
                        hdl_dict.get("label_names")
                    )
                    used_modules_dict[entity_name_used_in_arch]["target_architecture"] = [None] * len(
                        hdl_dict.get("label_names")
                    )
                    if language == "VHDL":
                    ...
                    for instance_type in used_modules_dict[entity_name_used_in_arch]["instance_types"]:
                        if instance_type in list_of_module_names_with_configuration:
                            instance_index = used_modules_dict[entity_name_used_in_arch]["instance_types"].index(
                                instance_type
                            )
                            configuration_index = list_of_module_names_with_configuration.index(instance_type)
                            if (
                                list_of_instance_names_with_configuration[configuration_index].lower() == "all"
                                or list_of_instance_names_with_configuration[configuration_index].lower()
                                == used_modules_dict[entity_name_used_in_arch]["label_names"][instance_index].lower()
                            ):
                                used_modules_dict[entity_name_used_in_arch]["target_library"][instance_index] = (
                                    list_of_target_libraries[configuration_index]
                                )
                                used_modules_dict[entity_name_used_in_arch]["target_module"][instance_index] = (
                                    list_of_target_modules[configuration_index]
                                )
                                used_modules_dict[entity_name_used_in_arch]["target_architecture"][instance_index] = (
                                    list_of_target_architectures[configuration_index]
                                )
            except FileNotFoundError:
                messagebox.showerror(
                    "Error in HDL-SCHEM-Editor",
                    "File " + file_name + " could not be opened during extracting hierarchy.",
                )
