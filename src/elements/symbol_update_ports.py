...
import constants
from elements import symbol_polygon_move
...
class SymbolUpdatePorts:
    def __init__(self, root, window, diagram_tab, symbol, symbol_define_ref):
        ...
        ports_to_add, _ = self.__search_list1_entries_in_list2(
            symbol_definition_upd["port_list"], symbol.symbol_definition["port_list"]
        )
        ...
        for port_entry_index in ports_to_add:
            ...
            port_name, port_direction, port_range = symbol.get_port_name_and_direction_and_range(
                new_port_entry["declaration"]
            )
            rectangle_coords = self.diagram_tab.canvas.coords(symbol.symbol_definition["rectangle"]["canvas_id"])
            new_polygon_coords, text_delta_x, text_anchor = self.__calculate_positions(
                port_direction, instance_updated, rectangle_coords
            )
            ...
            new_port_entry["canvas_id"] = self.diagram_tab.canvas.create_polygon(
                *new_polygon_coords,
                outline="black",
                fill=symbol_color,
                activefill="red",
            ...
            new_port_entry["canvas_id_text"] = self.diagram_tab.canvas.create_text(
                new_polygon_coords[4] + text_delta_x,
                new_polygon_coords[5],
                font=("Courier", self.window.design.get_font_size()),
                text=port_name + port_range,
                anchor=text_anchor,
            ...
            symbol.sym_bind_funcid_port_show[new_port_entry["canvas_id_text"]] = self.diagram_tab.canvas.tag_bind(
                new_port_entry["canvas_id_text"],
                "<Enter>",
                lambda event, canvas_id_text=new_port_entry["canvas_id_text"]: self.symbol.show_port_type(
                    canvas_id_text
                ),
            )
            symbol.sym_bind_funcid_port_hide[new_port_entry["canvas_id_text"]] = self.diagram_tab.canvas.tag_bind(
                new_port_entry["canvas_id_text"],
                "<Leave>",
                lambda event, canvas_id_text=new_port_entry["canvas_id_text"]: self.symbol.hide_port_type(
                    canvas_id_text
                ),
            )
        ...
        for change_entry in ports_type_change:
            ...
            port_name, port_direction_upd, port_range_upd = symbol.get_port_name_and_direction_and_range(
                declaration_upd
            )
            ...
            if port_direction_upd != port_direction:
                polygon_coords_new = self.__calculate_polygon_coords_for_direction_change(
                    change_entry, port_direction, port_direction_upd
                )
        ...
        for port_to_remove in ports_to_remove:
            ...
            symbol_definition_port_list_entries_to_remove.append(
                port_to_remove
            )  # Remove at the end to keep indices valid in the meantime.
        if (
            ports_to_remove
        ):  # must be done last, as it modifies the order of entries in self.symbol_definition["port_list"]
            new_port_list = []
    ...
    def _shift_symbol_rectangle_down(self, rectangle_coords):
        self.diagram_tab.canvas.coords(
            self.symbol.symbol_definition["rectangle"]["canvas_id"],
            rectangle_coords[0],
            rectangle_coords[1],
            rectangle_coords[2],
    ...
    def _shift_symbol_ports_down(self, rectangle_coords):
        for port_dict in self.symbol.symbol_definition["port_list"]:
            ...
            if polygon_touching_point_at_symbol[1] == rectangle_coords[3]:
                port_dict["coords"] = [
                    port_dict["coords"][i] + i % 2 * self.window.design.get_grid_size() for i in range(8)
                ]
                ...
                self.diagram_tab.canvas.coords(
                    port_dict["canvas_id_text"], coords_text[0], coords_text[1] + self.window.design.get_grid_size()
                )
    ...
    def _shift_symbol_entity_name_down(self):
        ...
        self.diagram_tab.canvas.coords(
            self.symbol.symbol_definition["entity_name"]["canvas_id"], entity_name_coords[0], entity_name_coords[1]
        )

    def _shift_symbol_instance_name_down(self):
        instance_name_coords = self.diagram_tab.canvas.coords(
            self.symbol.symbol_definition["instance_name"]["canvas_id"]
        )
        instance_name_coords[1] += self.window.design.get_grid_size()
        self.diagram_tab.canvas.coords(
            self.symbol.symbol_definition["instance_name"]["canvas_id"],
            instance_name_coords[0],
            instance_name_coords[1],
        )
    ...
    def __update_port_range_in_graphics(self, change_entry, new_text):
        self.diagram_tab.canvas.itemconfigure(
            self.symbol.symbol_definition["port_list"][change_entry["index_in_port_list"]]["canvas_id_text"],
            text=new_text,
        )

    def __update_polygon_in_graphics(self, change_entry, polygon_coords_new):
        self.diagram_tab.canvas.coords(
            self.symbol.symbol_definition["port_list"][change_entry["index_in_port_list"]]["canvas_id"],
            polygon_coords_new,
        )
        ...
        for index_in_port_list, port_entry in enumerate(port_list1):
            ...
            for index_in_port_list_upd, port_entry_upd in enumerate(port_list2):
                ...
                if port_declaration_upd == port_declaration:
                    ...
                    port_name_upd, port_direction_upd, _ = self.symbol.get_port_name_and_direction_and_range(
                        port_declaration_upd
                    )
                        ...
                        type_has_changed.append(
                            {
                                ...
                                "index_in_port_list_upd": index_in_port_list_upd,
                                "direction_change": direction_has_changed,
