...
class GenerateFrame:
    ...
    def __init__(
        self, root, window, diagram_tab, generate_definition
    ):  # generate_definition is an empty dictionary when called from GUI.
    ...
    def _draw_from_file(self):
        self.generate_definition["generate_rectangle_id"] = self.diagram_tab.canvas.create_rectangle(
            self.generate_definition["generate_rectangle_coords"],
            dash=(3, 5),
            activeoutline="red",
            activewidth=3,
            tag=(self.generate_definition["object_tag"], "schematic-element"),
        )
        self.generate_definition["generate_condition_id"] = self.diagram_tab.canvas.create_text(
            self.generate_definition["generate_rectangle_coords"][0:2],
    ...
    def _reject(self):
        if (
            "generate_rectangle_id" in self.generate_definition
        ):  # False after clicking to "new Generate"-button and not clicking into canvas.
    ...
    def _resize_rectangle_to_coordinates_from_generate_definition(self):
        self.diagram_tab.canvas.coords(
            self.generate_definition["generate_rectangle_id"], *self.generate_definition["generate_rectangle_coords"]
        )
    ...
    def _update_generate_definition(self):
        self.generate_definition["generate_rectangle_coords"] = self.diagram_tab.canvas.coords(
            self.generate_definition["generate_rectangle_id"]
        )
        self.generate_definition["generate_condition_coords"] = self.diagram_tab.canvas.coords(
            self.generate_definition["generate_condition_id"]
        )
        self.generate_definition["generate_condition"] = self.diagram_tab.canvas.itemcget(
            self.generate_definition["generate_condition_id"], "text"
        )

    def _add_bindings_for_generate_frame_handling(self):
        self.funcid_button1_rectangle = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_rectangle_id"], "<Button-1>", self._move_start
        )
        self.funcid_button1_condition = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_condition_id"], "<Button-1>", self._move_start_from_condition
        )
        self.funcid_enter_rectangle = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_rectangle_id"], "<Enter>", lambda event: self._at_enter()
        )
        self.funcid_enter_condition = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_condition_id"], "<Enter>", lambda event: self._at_enter()
        )
        self.funcid_leave_rectangle = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_rectangle_id"], "<Leave>", lambda event: self._at_leave()
        )
        self.funcid_leave_condition = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_condition_id"], "<Leave>", lambda event: self._at_leave()
        )
        self.funcid_button1_double = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_condition_id"], "<Double-Button-1>", lambda event: self.edit()
        )

    def _remove_bindings_from_generate_frame(self):
        self.diagram_tab.canvas.tag_unbind(
            self.generate_definition["generate_rectangle_id"], "<Button-1>", self.funcid_button1_rectangle
        )
        self.diagram_tab.canvas.tag_unbind(
            self.generate_definition["generate_condition_id"], "<Button-1>", self.funcid_button1_condition
        )
        self.diagram_tab.canvas.tag_unbind(
            self.generate_definition["generate_rectangle_id"], "<Enter>", self.funcid_enter_rectangle
        )
        self.diagram_tab.canvas.tag_unbind(
            self.generate_definition["generate_condition_id"], "<Enter>", self.funcid_enter_condition
        )
        self.diagram_tab.canvas.tag_unbind(
            self.generate_definition["generate_rectangle_id"], "<Leave>", self.funcid_leave_rectangle
        )
        self.diagram_tab.canvas.tag_unbind(
            self.generate_definition["generate_condition_id"], "<Leave>", self.funcid_leave_condition
        )
        self.diagram_tab.canvas.tag_unbind(
            self.generate_definition["generate_condition_id"], "<Double-Button-1>", self.funcid_button1_double
        )
    ...
    def _move_start_from_condition(self, event):
        ...
        self.func_id_motion = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_condition_id"], "<Motion>", lambda event: self._move_to(event, "middle")
        )
        self.func_id_button_release = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_condition_id"],
            "<ButtonRelease-1>",
            lambda event: self._move_end("condition"),
        )
    ...
    def _move_start(self, event):
        ...
        self.func_id_motion = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_rectangle_id"],
            "<Motion>",
            lambda event: self._move_to(event, touching_point),
        )
        self.func_id_button_release = self.diagram_tab.canvas.tag_bind(
            self.generate_definition["generate_rectangle_id"],
            "<ButtonRelease-1>",
            lambda event: self._move_end("rectangle"),
        )
    ...
    def _get_touching_point(self, rectangle_coords):
        ...
        if self.event_x < rectangle_coords[0] + distance and self.event_y < rectangle_coords[1] + distance:
            return "top_left"
        if self.event_x > rectangle_coords[2] - distance and self.event_y < rectangle_coords[1] + distance:
            return "top_right"
        if self.event_x < rectangle_coords[0] + distance and self.event_y > rectangle_coords[3] - distance:
            return "bottom_left"
        if self.event_x > rectangle_coords[2] - distance and self.event_y > rectangle_coords[3] - distance:
            return "bottom_right"
        return "middle"
    ...
    def _move_end(self, moved_object):
        if moved_object == "rectangle":
            self.diagram_tab.canvas.tag_unbind(
                self.generate_definition["generate_rectangle_id"], "<Motion>", self.func_id_motion
            )
            self.diagram_tab.canvas.tag_unbind(
                self.generate_definition["generate_rectangle_id"], "<ButtonRelease-1>", self.func_id_button_release
            )
        else:
            self.diagram_tab.canvas.tag_unbind(
                self.generate_definition["generate_condition_id"], "<Motion>", self.func_id_motion
            )
            self.diagram_tab.canvas.tag_unbind(
                self.generate_definition["generate_condition_id"], "<ButtonRelease-1>", self.func_id_button_release
            )
        if self.generate_definition["generate_rectangle_coords"] != self.diagram_tab.canvas.coords(
            self.generate_definition["generate_rectangle_id"]
        ) or self.generate_definition["generate_condition_coords"] != self.diagram_tab.canvas.coords(
            self.generate_definition["generate_condition_id"]
        ):
            self.store_item(push_design_to_stack=True, signal_design_change=True)
    ...
    def edit(self):
        ...
        self.diagram_tab.canvas.create_window(
            self.generate_definition["generate_rectangle_coords"][0:2],
            window=self.condition_entry_text_box,
            anchor="sw",
            tag="entry-window",
        )
    ...
    def _update_generate_condition(self):
        ...
        if new_text != self.generate_definition["generate_condition"]:
            ...
            self.diagram_tab.canvas.itemconfigure(
                self.generate_definition["generate_condition_id"], text=self.generate_definition["generate_condition"]
            )
            self.store_item(push_design_to_stack=True, signal_design_change=True)
