...
class Block:
    ...
    def _draw_once_at_event_location(self, event, rect_color):
        ...
        self.event_x -= (
            self.event_x % self.diagram_tab.design.get_grid_size() - self.diagram_tab.design.get_grid_size() / 2
        )
        self.event_y -= (
            self.event_y % self.diagram_tab.design.get_grid_size() - self.diagram_tab.design.get_grid_size() / 2
        )
        ...
        self.func_id_motion = self.diagram_tab.canvas.bind(
            "<Motion>", lambda event: self._move_to(event, references_to_connected_wires, "middle")
        )

    def _draw(self, rect_coords, text_coords, rect_color, text):
        self.canvas_id = self.diagram_tab.canvas.create_text(
            text_coords[0],
            text_coords[1],
            text=text,
            anchor="nw",
        ...
        self.rectangle_reference = block_rectangle.BlockRectangle(
    ...
    def _get_coords_expanded_to_grid(self, enclosing_rectangle, old_height_before_zoom=0, old_width_before_zoom=0):
        ...
        remainder_1 = (
            enclosing_rectangle[0] % self.window.design.get_grid_size()
        )  # Remember: the result of the %-operation gets the sign from the divisor!
        if remainder_1 != 0:
        ...
        if remainder_2 != 0:
            if remainder_1 > remainder_2:  # increase by remainder_1 is bigger than decrease by remainder_2
                new_enclosing_rectangle[2] -= (
                    remainder_2  # decrease the rectangle, but in sum an increase for the rectangle.
                )
            else:
        ...
        if (
            abs(new_enclosing_rectangle[2] - new_enclosing_rectangle[0] - self.window.design.get_grid_size())
            <= self.window.design.get_grid_size() / 10
        ):
            new_enclosing_rectangle[2] += self.window.design.get_grid_size()
        if (
            abs(new_enclosing_rectangle[3] - new_enclosing_rectangle[1] - self.window.design.get_grid_size())
            <= self.window.design.get_grid_size() / 10
        ):
        ...
        if old_height_before_zoom != 0:  # Only true, when zoom is involved.
            ...
            while int(new_height) < int(old_height_before_zoom):
                new_enclosing_rectangle[3] += self.window.design.get_grid_size()
                new_height = (
                    new_enclosing_rectangle[3] - new_enclosing_rectangle[1]
                ) / self.window.design.get_grid_size()
        if old_width_before_zoom != 0:  # Only true, when zoom is involved.
            ...
            while int(new_width) < int(old_width_before_zoom):
                new_enclosing_rectangle[2] += self.window.design.get_grid_size()
                new_width = (
                    new_enclosing_rectangle[2] - new_enclosing_rectangle[0]
                ) / self.window.design.get_grid_size()
    ...
    def _reject(self):
        if self.rectangle_reference is not None:
            ...
            del (
                self
            )  # Once the last reference to an object is deleted, the object will be removed by garbage collection.
    ...
    def _move_start(self, event):
        ...
        self.func_id_motion = self.diagram_tab.canvas.tag_bind(
            ...
            "<Motion>",
            lambda event: self._move_to(event, references_to_connected_wires, touching_point),
        )
        self.func_id_button_release = self.diagram_tab.canvas.tag_bind(
            self.rectangle_canvas_id,
            "<ButtonRelease-1>",
            lambda event: self._move_end(references_to_connected_wires, touching_point),
        )
        ...
        for canvas_id in list_overlapping:
            if self.diagram_tab.canvas.type(canvas_id) == "line" and "grid_line" not in self.diagram_tab.canvas.gettags(
                ...
                if (
                    ...
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "first", "left")
                    )
                elif (
                    self._coords_are_equal(line_coords[0], block_coords[2])  # right of block
                    and block_coords[1] <= line_coords[1] <= block_coords[3]
                ):  #        and # inside block
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "first", "right")
                    )
                elif (
                    self._coords_are_equal(line_coords[1], block_coords[1])  # top of block
                    and block_coords[0] <= line_coords[0] <= block_coords[2]
                ):  #       and # inside block
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "first", "top")
                    )
                elif (
                    self._coords_are_equal(line_coords[1], block_coords[3])  # bottom of block
                    and block_coords[0] <= line_coords[0] <= block_coords[2]
                ):  #       and # inside block
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "first", "bottom")
                    )
                elif (
                    self._coords_are_equal(line_coords[-2], block_coords[0])
                    and block_coords[1] <= line_coords[-1] <= block_coords[3]
                ):  #  and
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "last", "left")
                    )
                elif (
                    self._coords_are_equal(line_coords[-2], block_coords[2])
                    and block_coords[1] <= line_coords[-1] <= block_coords[3]
                ):  #  and
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "last", "right")
                    )
                elif (
                    self._coords_are_equal(line_coords[-1], block_coords[1])
                    and block_coords[0] <= line_coords[-2] <= block_coords[2]
                ):  #  and
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "last", "top")
                    )
                elif (
                    self._coords_are_equal(line_coords[-1], block_coords[3])
                    and block_coords[0] <= line_coords[-2] <= block_coords[2]
                ):  #  and
                    references_to_connected_wires.append(
                        (self.diagram_tab.design.get_references([canvas_id])[0], "last", "bottom")
                    )
                else:
    ...
    def _add_bindings_to_block(self):
        self.sym_bind_funcid_button = self.diagram_tab.canvas.tag_bind(
            self.rectangle_canvas_id, "<Button-1>", self._move_start
        )
        self.sym_bind_funcid_enter1 = self.diagram_tab.canvas.tag_bind(
            self.rectangle_canvas_id, "<Enter>", lambda event: self._at_enter()
        )
        self.sym_bind_funcid_leave1 = self.diagram_tab.canvas.tag_bind(
        ...
        self.sym_bind_funcid_edit_ex = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Control-e>", lambda event: self._edit_ext()
        )
    ...
    def _create_bindings_for_insertion_at_canvas(self, rect_color):
        self.func_id_motion = self.diagram_tab.canvas.bind(
            "<Motion>", lambda event, rect_color=rect_color: self._draw_once_at_event_location(event, rect_color)
        )
    ...
    def _at_enter(self):
        if not self.diagram_tab.canvas.find_withtag("selected"):
            ...
            self.funcid_delete = self.diagram_tab.canvas.bind(
                "<Delete>", lambda event: self._delete_text_and_rectangle()
            )
    ...
    def _position_insertion_cursor_under_mouse(self, event):
        ...
        self.diagram_tab.canvas.after_idle(
            lambda: self.block_edit_ref.text_edit_widget.mark_set("insert", f"@{delta_x},{delta_y}")
        )
