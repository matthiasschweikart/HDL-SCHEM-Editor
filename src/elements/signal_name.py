...
class SignalName:
    """Class for inserting a signal name into the canvas and for editing the signal name."""
    ...
    def __init__(
        ...
        self.canvas_id = diagram_tab.canvas.create_text(
            coords,
        ...
        self._process_new_declaration(declaration, False)  # The given declaration overwrites the "dummy".
        self.store_item(push_design_to_stack=False, signal_design_change=False)
        self._add_bindings_to_signal_name()

    def select_item(self):
        """Highlights the signal name, which is needed for selection."""
        self._highlight()
        self._remove_bindings_from_signal_name()

    def unselect_item(self):
        """Removes the highlight from the signal name."""
        self._unhighlight()
        self._add_bindings_to_signal_name()

    def get_declaration(self):
        """Returns the declaration of the signal name."""
    ...
    def get_canvas_id(self):
        """Returns the canvas id of the signal name."""
        return self.canvas_id

    def store_item(self, push_design_to_stack, signal_design_change):
        """Stores the signal name in the canvas dictionary, which is needed for undo/redo and file write."""
        coords = self.diagram_tab.canvas.coords(self.canvas_id)
        self.design.store_signal_name_in_canvas_dictionary(
            self.canvas_id,
            self,
            coords,
            self.angle,
    ...
    def _add_bindings_to_signal_name(self):
        self.sym_bind_funcid_button1 = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Button-1>", self._move_start_signal_name
        )
        self.sym_bind_funcid_dbutton1 = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Double-Button-1>", lambda event: self._edit_signal_name()
        )
        self.sym_bind_funcid_button3 = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<ButtonRelease-3>", lambda event: self._rotate_after_idle()
        )
    ...
    def _rotate_after_idle(self):
        ...
        # and background-menu both at Button-3). The background-menu will only be drawn if the mouse pointer is not over
        # any other object. So when a signal-name is rotated by Button-3 no background-menu should show, as the
        # mouse-pointer is over the signal-name. But as (for unknown reasons) always first the signal-name is rotated
        # and disappears from the mouse-pointer, afterwards always the background-menu showed.
        ...
        self.diagram_tab.canvas.after_idle(self._rotate)
    ...
    def _remove_bindings_from_signal_name(self):
    ...
    def _move_start_signal_name(self, event):
        self.func_id_motion = self.diagram_tab.canvas.tag_bind(self.canvas_id, "<Motion>", self._move_to)
        self.func_id_button_release = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<ButtonRelease-1>", self._move_end
        )
        ...
        for canvas_item in self.diagram_tab.canvas.find_withtag(self.wire_tag):
            if self.diagram_tab.canvas.type(canvas_item) == "line":
                ...
                self.anchor_line = self.diagram_tab.canvas.create_line(
                    self.anchor_line_coords[0],
                    self.anchor_line_coords[1],
                    self.event_x,
                    self.event_y,
                    dash=(2, 3),
                    fill="red",
    ...
    def _move_to(self, event):
        ...
        self.diagram_tab.canvas.coords(
            self.anchor_line, self.anchor_line_coords[0], self.anchor_line_coords[1], self.event_x, self.event_y
        )

    def _move_end(self, _):
    ...
    def _edit_signal_name(self):
        ...
        self.text_box.bind("<Key>", lambda event: self._increase_length())
    ...
    def update_signal_name(self, all_signals):
        """Updates the signal name, which is needed when the user finishes editing by pressing return."""
        new_declaration = self.text_box.get()
        if self.design.get_language() == "VHDL" and (
            new_declaration.find(":") == -1 or new_declaration.startswith(":") or new_declaration.endswith(":")
        ):
        ...
        elif self.design.get_language() == "Verilog" and (
            (new_declaration.find("reg") == -1 and new_declaration.find("wire") == -1)
            or new_declaration.find("(") != -1
            or new_declaration.find(")") != -1
        ):
            messagebox.showerror(
                "Error in HDL-SCHEM-Editor",
        ...
        elif self.design.get_language() == "SystemVerilog" and (
            (
                new_declaration.find("reg") == -1
                and new_declaration.find("wire") == -1
                and new_declaration.find("logic") == -1
            )
            or new_declaration.find("(") != -1
            or new_declaration.find(")") != -1
        ):
            ...
            self._process_new_declaration(new_declaration, all_signals)
            for canvas_item in self.diagram_tab.canvas.find_withtag(self.wire_tag):
                if self.diagram_tab.canvas.type(canvas_item) == "line":
                    ...
                    )  # Must be stored, as the width of the wire may have changed.
        self.delete_entry_window()

    def delete_entry_window(self):
        """Deletes entry window of a signal name edit."""
        self.design.signal_name_edit_list_remove(
            self
        )  # Needed if an entry window is open, when the wire and the signal-name are deleted.
    ...
    def _get_part_to_show_from_declaration(self, declaration):
        signal_name, signal_sub_range, signal_type, _, _, signal_record_slice = (
            hdl_generate_functions.HdlGenerateFunctions.split_declaration(declaration, self.design.get_language())
        )
        if signal_sub_range != "":
            visible_range = signal_sub_range
            # When a sub_range exists the complete range must be removed from the
            # signal name (is only part of the signal_name in Verilog designs):
            ...
            # The check for " range " is only a check for VHDL, it is not clear if
            # something similar is needed for Verilog:
    ...
    def get_object_tag(self):
        """Returns the tag of the signal name, which is needed for notebook_diagram_tab.__complete_selection()."""
        return self.wire_tag + "_signal_name"

    def _process_new_declaration(self, new_declaration, all_signals):
        ...
        #     From the new declaration only signal name and signal range are used for this wire,
        #     all other information is copied from the already existing wire.
        #     From the new declaration the signal type is not used (if it would be used, then the type of the new
        #     declaration must be adapted by the user to the type of the existing wire in most of the cases).
        # If the signal name was changed and CTRL was pressed (All other wires which have the old signal name shall
        # also get the changes):
        ...
        #  Else (Reconnect net):
        #     This wire is now connected to an already existing wire and the other signals with the old signal name are
        #     also connected to the already existing wire.
        #     From the new declaration only wire name and wire range are used for this wire, type, init, comment are
        #     copied from the already existing wire.
        #     From the new declaration only wire name is used for all other wires with old name, range is kept
        #     unchanged, type, init, comment are copied from the wire connected to.
        new_signal_name, new_sub_range, new_signal_type, new_comment, new_init, new_signal_record_slice = (
            hdl_generate_functions.HdlGenerateFunctions.split_declaration(new_declaration, self.design.get_language())
        )
        ...
        old_signal_name, _, _, _, _, _ = hdl_generate_functions.HdlGenerateFunctions.split_declaration(
            self.declaration, self.design.get_language()
        )
        if new_signal_name == old_signal_name:  # Only the other properties of wire (or net) are changed.
            ...
            self._propagate_all_but_subrange_to_other_wires_with_old_signal_name(
                old_signal_name, new_signal_name, new_signal_type, new_init, new_comment
            )
        else:
            other_references = self._get_signal_name_references_with_name(new_signal_name)
            if (
                not other_references
            ):  # No other wire with new_signal_name exists, so rename wire (and update other properties).
                self.change_declaration(new_declaration)
                ...
                existing_signal_type, existing_init, existing_comment = (
                    self._get_type_and_init_and_comment_from_a_wire_which_already_has_the_new_name(
                ...
                if self.design.get_language() == "VHDL":
                    modified_declaration = (
                        ...
                        + new_signal_record_slice
                        + " : "
                        + existing_signal_type
                        + existing_init
                        + existing_comment
                    )
                else:  # Verilog
                    modified_declaration = (
                        existing_signal_type + " " + new_signal_name + new_sub_range + existing_comment
                    )
                self.change_declaration(modified_declaration)
                if all_signals:  # Connect complete net to other wire.
                    self._propagate_all_but_subrange_to_other_wires_with_old_signal_name(
                        old_signal_name, new_signal_name, existing_signal_type, existing_init, existing_comment
                    )

    def _get_type_and_init_and_comment_from_a_wire_which_already_has_the_new_name(
        self, signal_name_references_with_new_name, new_signal_type, new_comment
    ):
        other_declaration = signal_name_references_with_new_name[0].get_declaration()  # Read only 1 declaration.
        _, _, new_signal_type, new_comment, new_init, _ = hdl_generate_functions.HdlGenerateFunctions.split_declaration(
            other_declaration, self.design.get_language()
        )
        return new_signal_type, new_init, new_comment

    def _propagate_all_but_subrange_to_other_wires_with_old_signal_name(
        self, old_signal_name, new_signal_name, new_signal_type, new_init, new_comment
    ):
        signal_name_references_with_old_name = self._get_signal_name_references_with_name(old_signal_name)
        if signal_name_references_with_old_name:
            for signal_name_reference_with_old_name in signal_name_references_with_old_name:
                if signal_name_reference_with_old_name != self:
                    ...
                    _, other_signal_range, _, _, _, other_signal_record_slice = (
                    ...
                    if new_comment != "" and not new_comment.startswith(" "):
                        new_comment = " " + new_comment
                    if self.design.get_language() == "VHDL":
                        signal_name_reference_with_old_name.change_declaration(
                            new_signal_name
                            + other_signal_range
                            + other_signal_record_slice
                            + " : "
                            + new_signal_type
                            + new_init
                            + new_comment
                        ...
                        signal_name_reference_with_old_name.change_declaration(
                            new_signal_type + " " + new_signal_name + other_signal_range + new_comment
                        )

    def _get_signal_name_references_with_name(self, signal_name):
        ...
        for signal_name_reference in signal_name_references:
            other_signal_name, _, _, _, _, _ = hdl_generate_functions.HdlGenerateFunctions.split_declaration(
                signal_name_reference.declaration, self.design.get_language()
            )
            if other_signal_name == signal_name:
                identical_name_references.append(signal_name_reference)
        return identical_name_references

    def set_declaration_if_signal_names_differ(self, new_declaration):
        """If the signal name of the new declaration differs from the old one,
        then the declaration of this signal name is changed to the new declaration."""
        # called from WireMove.__change_signal_name_if_connected_to_other_wire()
        old_signal_name, _, _, _, _, _ = hdl_generate_functions.HdlGenerateFunctions.split_declaration(
            self.declaration, self.design.get_language()
        )
        new_signal_name, _, _, _, _, _ = hdl_generate_functions.HdlGenerateFunctions.split_declaration(
    ...
    def change_declaration(self, new_declaration):
        """Changes the declaration of the signal name."""
        self.declaration = new_declaration
        self.diagram_tab.canvas.itemconfigure(
            self.canvas_id, text=self._get_part_to_show_from_declaration(new_declaration)
        )
        self._adapt_line_thickness()
        self.store_item(push_design_to_stack=False, signal_design_change=False)

    def _adapt_line_thickness(self):
        canvas_id_line = None
        list_of_canvas_ids = self.diagram_tab.canvas.find_withtag(self.wire_tag)
        for canvas_id in list_of_canvas_ids:
            if self.diagram_tab.canvas.type(canvas_id) == "line" and "grid_line" not in self.diagram_tab.canvas.gettags(
                canvas_id
            ):
    ...
    def add_pasted_tag_to_all_canvas_items(self):
        """Adds the tag "pasted_tag" to all canvas items of this signal name, which is needed for pasting."""
        self.diagram_tab.canvas.addtag_withtag("pasted_tag", self.canvas_id)

    def adapt_coordinates_by_factor(self, factor):
        """Adapts the coordinates of the signal name by a factor, which is needed for zooming."""
        coords = self.diagram_tab.canvas.coords(self.canvas_id)
