        ...
        try:
            ...
            if filename.endswith(".vhd"):
                ...
                if "active__architecture" in hdl_schem_editor_design_dictionary:
                    ...
                    hdl_schem_editor_design_dictionary = hdl_schem_editor_design_dictionary[
                        hdl_schem_editor_design_dictionary["active__architecture"]
                    ]
                else:
                    architecture_list = []
                ...
                additional_sources = hdl_schem_editor_design_dictionary["additional_sources"].split(
                    ","
                )  # Conversion from comma-separated string into list
                ...
                if language_of_instance == "VHDL":
                    ...
                    interface_generics_parsed = vhdl_parsing.VhdlParser(
                        window.design.get_generics_from_design_dictionary(hdl_schem_editor_design_dictionary),
                        "generics",
                    )
                    ...
                    interface_generics_parsed = verilog_parsing.VerilogParser(
                        window.design.get_generics_from_design_dictionary(hdl_schem_editor_design_dictionary),
                        "parameter_list",
                    )
                    library_names = ""
                ...
                try:  # "try" is needed because an old file hfe-format exists.
                    ...
                    if language_of_instance == "VHDL":
                        interface_package_parsed = vhdl_parsing.VhdlParser(
                            hdl_fsm_editor_design_dictionary["interface_package"], "entity_context"
                        )
                        interface_ports_parsed = vhdl_parsing.VhdlParser(
                            hdl_fsm_editor_design_dictionary["interface_ports"], "ports"
                        )
                        interface_generics_parsed = vhdl_parsing.VhdlParser(
                            hdl_fsm_editor_design_dictionary["interface_generics"], "generics"
                        )
                        library_names = interface_package_parsed.get("entity_library_name")
                        package_names = interface_package_parsed.get("package_name")
                        ...
                        interface_ports_parsed = verilog_parsing.VerilogParser(
                            hdl_fsm_editor_design_dictionary["interface_ports"], "port_region"
                        )
                        interface_generics_parsed = verilog_parsing.VerilogParser(
                            hdl_fsm_editor_design_dictionary["interface_generics"], "parameter_list"
                        )
                        library_names = ""
                        package_names = ""
                        entity_name = hdl_fsm_editor_design_dictionary["modulename"]
                        ...
                        generic_definition = interface_generics_parsed.get("parameter_definition")
                        generic_types = []
                    generic_definition = list_separation_check.ListSeparationCheck(
                        generic_definition, language_of_instance
                    ).get_fixed_list()
                except json.JSONDecodeError:
                    messagebox.showerror(
                        "Error in HDL-SCHEM-Editor", "File " + filename + " is not a .hfe file in JSON format."
                    )
                    return
            ...
            messagebox.showerror(
                "Error in HDL-SCHEM-Editor", "File " + filename + " could not be found by SymbolDefine."
            )
