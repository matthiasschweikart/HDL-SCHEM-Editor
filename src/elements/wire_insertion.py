...
class Wire:
    ...
    def _end_wire(self, width):
        ...
        if wire_coords != []:
            ...
            signal_name.SignalName(
                ...
                coords=[wire_coords[0] + self.window.design.get_grid_size(), wire_coords[1]],
                angle=0,
        ...
        if signal_name_reference is not None:
            ...
            if self.window.design.get_language() == "VHDL":
                if width == 1:
                    ...
                    signal_declaration = (
                        "signal_" + str(self.window.design.get_wire_id()) + " : std_logic_vector(7 downto 0)"
                    )
            else:
                if width == 1:
    ...
    def _wire_is_connected_to_2_wires(self, wire_coords):
        ...
        for end_point in end_points:
            overlapping_canvas_ids = self.diagram_tab.canvas.find_overlapping(
                end_point[0] - 1, end_point[1] - 1, end_point[0] + 1, end_point[1] + 1
            )
    ...
    def _add_dot_if_needed_at(self, dot_x, dot_y):
        ...
        for overlapping_signal_name in overlapping_signal_names:
            if overlapping_signal_name != name_of_signal:
                return None
        if line_found >= 2:
            dot_ref = dot_insertion.Dot(
                self.window, push_design_to_stack=False, coords=[dot_x, dot_y], line_size=line_size
            )
            return dot_ref
        return None
    ...
    def _remove_identical_wire_points(self, wire_coords):
        ...
        for index in range(len(wire_coords) // 2 - 1):
            if not (
                abs(wire_coords[2 * index + 2] - wire_coords[2 * index]) < self.window.design.get_grid_size() / 10
                and abs(wire_coords[2 * index + 3] - wire_coords[2 * index + 1])
                < self.window.design.get_grid_size() / 10
            ):
    ...
    def _remove_3_wire_points_in_a_row(self, wire_coords):
        ...
        for index in range(len(wire_coords) // 2 - 2):
            if not (
                (
                    abs(wire_coords[2 * index] - wire_coords[2 * index + 2]) < self.window.design.get_grid_size() / 10
                    and abs(wire_coords[2 * index + 2] - wire_coords[2 * index + 4])
                    < self.window.design.get_grid_size() / 10
                )
                or (
                    abs(wire_coords[2 * index + 1] - wire_coords[2 * index + 3])
                    < self.window.design.get_grid_size() / 10
                    and abs(wire_coords[2 * index + 3] - wire_coords[2 * index + 5])
                    < self.window.design.get_grid_size() / 10
                )
            ):
                wire_coords_mod.append(wire_coords[2 * index + 2])
    ...
    def signal_name_not_near_segment(self, segment_to_move, wire_coords):  # segment_to_move = 0, 1, 2, ...
        ...
        if (
            segment_coords[0] - segment_coords[2]
            > self.window.design.get_grid_size() / 2  # horizontal with wrong order
            or segment_coords[1] - segment_coords[3] > self.window.design.get_grid_size() / 2
        ):  # vertical with wrong order
    ...
    def _signal_name_near_wire_end_point(
        ...
        return (
            ...
            <= signal_name_anchor[0]
            <= segment_coords[0] + self.diagram_tab.design.get_grid_size()
            and segment_coords[1] - self.diagram_tab.design.get_grid_size()
            <= signal_name_anchor[1]
            <= segment_coords[1] + self.diagram_tab.design.get_grid_size()
        ) or (
            first_or_last == "last"
            and segment_coords[2] - self.diagram_tab.design.get_grid_size()
            <= signal_name_anchor[0]
            <= segment_coords[2] + self.diagram_tab.design.get_grid_size()
    ...
    def determine_connected_endpoints(self, wire_coords):
        ...
        overlapping_ids = self.diagram_tab.canvas.find_overlapping(
            ...
            wire_coords[1] - 0.6 * self.window.design.get_grid_size(),
            wire_coords[0] + 0.6 * self.window.design.get_grid_size(),
            wire_coords[1] + 0.6 * self.window.design.get_grid_size(),
        )
        for canvas_id in overlapping_ids:
            if canvas_id != self.canvas_id:
                if self.diagram_tab.canvas.type(
                    canvas_id
                    ...
                    polygon_coords = self.diagram_tab.canvas.coords(
                        canvas_id
                    )  # The first point is the wire connection point.
                    if (
                        abs(wire_coords[0] - polygon_coords[0]) <= 0.6 * self.window.design.get_grid_size()
                        and abs(wire_coords[1] - polygon_coords[1]) <= 0.6 * self.window.design.get_grid_size()
                    ):  # Check if the wire ends at the connection-point of the polygon.
                        end_point_connected = "first"
                    # else:
                    ...
                    self.diagram_tab.canvas.type(canvas_id) == "rectangle"
                    and self.window.design.get_schematic_element_type_of(canvas_id) == "block-rectangle"
                ):
                    end_point_connected = "first"
        overlapping_ids = self.diagram_tab.canvas.find_overlapping(
            wire_coords[-2] - 0.6 * self.window.design.get_grid_size(),
            wire_coords[-1] - 0.6 * self.window.design.get_grid_size(),
            wire_coords[-2] + 0.6 * self.window.design.get_grid_size(),
            wire_coords[-1] + 0.6 * self.window.design.get_grid_size(),
        )
        for canvas_id in overlapping_ids:
            if canvas_id != self.canvas_id:
                if self.diagram_tab.canvas.type(
                    ...
                    if (
                        abs(wire_coords[-2] - polygon_coords[0]) <= 0.6 * self.window.design.get_grid_size()
                        and abs(wire_coords[-1] - polygon_coords[1]) <= 0.6 * self.window.design.get_grid_size()
                    ):
                        if end_point_connected == "none":
                            end_point_connected = "last"
    ...
    def move_wire_end_point(self, movement_phase, first_or_last, delta_x, delta_y):
        ...
        if (
            self.segment_direction_at_symbol_move[first_or_last] is None
        ):  # True for "first" or "last" only at the first call of move_wire_end_point.
        ...
        if len(wire_coords) == 4:  # The wire is a straight line.
            if (
                self.endpoints_connected_at_symbol_move is None
            ):  # True at each first call of move_wire_end_point for a moving.
                # Check if the other end of the line is also connected to a symbol:
                self.endpoints_connected_at_symbol_move = self.determine_connected_endpoints(
                    wire_coords
                )  # Will get one of these values: "both", "first", "last"
            if (
                self.endpoints_connected_at_symbol_move != "both"
            ):  # A straight line with one open end is connected to a symbol, so both ends must be moved.
        ...
        if movement_phase != "last_time":
            ...
            self.store_item(
                push_design_to_stack=False, signal_design_change=True
    ...
    def remove_unnecessary_points(self, coords):
        ...
        if len(coords) >= 6:
            for index in range(len(coords) // 2 - 2):
                if not (
                    (
                        abs(coords_in_grid_size[2 * index] - coords_in_grid_size[2 * index + 2]) < 0.1
                        and abs(coords_in_grid_size[2 * index + 2] - coords_in_grid_size[2 * index + 4]) < 0.1
                    )
                    or (
                        abs(coords_in_grid_size[2 * index + 1] - coords_in_grid_size[2 * index + 3]) < 0.1
                        and abs(coords_in_grid_size[2 * index + 3] - coords_in_grid_size[2 * index + 5]) < 0.1
                    )
                ):
    ...
    def _add_bindings_to_wire(self):
        ...
        self.wire_bind_funcid_button = self.diagram_tab.canvas.tag_bind(
            self.canvas_id,
            "<Button-1>",
            lambda event: wire_move.WireMove(
                event, self.window, self.diagram_tab, self, self.canvas_id, self.wire_tag, shift_was_pressed=False
            ),
        )
        self.wire_bind_funcid_cbutton = self.diagram_tab.canvas.tag_bind(
            self.canvas_id,
            "<Shift-Button-1>",
            lambda event: wire_move.WireMove(
                event, self.window, self.diagram_tab, self, self.canvas_id, self.wire_tag, shift_was_pressed=True
            ),
        )
    ...
    def _show_signal_declaration(self, event, canvas_id):
        if (
            ...
            self.signal_name_showed_at_enter = self.diagram_tab.canvas.create_text(
                self.diagram_tab.canvas.canvasx(event.x) + self.diagram_tab.design.get_grid_size(),
            ...
            self.background_rectangle = self.diagram_tab.canvas.create_rectangle(
                self.diagram_tab.canvas.bbox(self.signal_name_showed_at_enter), fill="white"
            )
    ...
    def store_item(self, push_design_to_stack, signal_design_change):
        ...
        self.window.design.store_wire_in_canvas_dictionary(
            self.canvas_id, self, coords, tags, arrow, width, push_design_to_stack, signal_design_change
        )
