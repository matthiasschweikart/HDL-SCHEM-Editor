...
class SymbolInsertion:  # used by SymbolDefine
    def __init__(
        ...
        self.input_polygon_coords = [
            0,
            0,  # the first point is the connectionpoint at the grid.
            0,
    ...
    def draw_symbol(self):  # called by symbol_reading
        ...
        self.event_x = self.diagram_tab.canvas.canvasx(
            self.diagram_tab.canvas.winfo_pointerx() - self.diagram_tab.canvas.winfo_rootx()
        )
    ...
    def __create_symbol_definition(self):
        ...
        if "module_library" in self.properties:
            symbol_definition["configuration"] = {
                "library": self.properties["module_library"],
                "config_statement": "None",
            }
        else:
        ...
        symbol_definition["entity_name"] = {
            "canvas_id": None,
            "coords": [
                self.event_x + self.window.design.get_grid_size(),
                self.event_y - 1.5 * self.window.design.get_grid_size(),
            ],
            "name": self.properties["entity_name"],
        }
        all_instance_names = self.window.design.get_all_instance_names()
        symbol_definition["instance_name"] = {
            "canvas_id": None,
            "coords": [
                self.event_x + self.window.design.get_grid_size(),
                self.event_y - 0.5 * self.window.design.get_grid_size(),
            ],
        ...
        symbol_definition["rectangle"] = {
            "coords": [
                self.event_x + 0.5 * self.window.design.get_grid_size(),
                self.event_y - height,
        ...
        symbol_definition["port_list"] = (
            port_location_list_inputs + port_location_list_inouts + port_location_list_outputs
        )
        ...
        if self.properties["language"] == "VHDL":
            for line in generic_definition_lines:
                if line != "":
                    ...
                    line = re.sub(
                        r"\s*:.*:=\s*", " => ", line
                    )  # with init value: replace from "type declaration" to ":=" by "=>"
                    line = re.sub(
                        r"\s*:.*", " => ?", line
                    )  # without init value: replace from "type declaration" to end of line by "=>"
