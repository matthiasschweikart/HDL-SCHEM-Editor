...
class WireMove:
        ...
        if touched_segment == "middle":
            ...
            if direction_of_segment == "horizontal":
                ...
                if how_to_move_the_signal_name != "not":
                    how_to_move_the_signal_name = (
                        "move_y"  # A "middle" segment shall not move the signalname depending from the touching point.
                    )
            else:
                ...
                if how_to_move_the_signal_name != "not":
                    how_to_move_the_signal_name = (
                        "move_x"  # A "middle" segment shall not move the signalname depending from the touching point.
                    )
            ...
            if end_point_connected == "none" and number_of_segments == 1:
                if not shift_was_pressed:  # No new edge is inserted in the wire.
                    ...
                    if touched_segment == "first_horizontal":
                        if (
                            ...
                            and abs(self.event_y - wire_coords[1]) < self.window.design.get_grid_size() / 2
                        ):  # The first point of the wire is moved.
                            x_coordinate_to_change[1] = False
                        elif (
                            abs(self.event_x - wire_coords[-2]) < self.window.design.get_grid_size() / 2
                            and abs(self.event_y - wire_coords[-1]) < self.window.design.get_grid_size() / 2
                ...
                if end_point_connected == "none":  # number_of_segments > 1
                    ...
                    if end_point_connected == "first":
                        ...
                        if (
                            touched_segment == "first_horizontal"
                            and abs(self.event_x - wire_coords[0]) < abs(self.event_x - wire_coords[2])
                        ) or (
                            touched_segment == "first_vertical"
                            and abs(self.event_y - wire_coords[1]) < abs(self.event_y - wire_coords[3])
                        ):  # Disconnect
                        ...
                        if (
                            touched_segment == "first_horizontal"
                            and abs(self.event_x - wire_coords[2]) < abs(self.event_x - wire_coords[0])
                        ) or (
                            touched_segment == "first_vertical"
                            and abs(self.event_y - wire_coords[3]) < abs(self.event_y - wire_coords[1])
                        ):  # Disconnect
    ...
    def _determine_segment_to_move(self, number_of_segments, wire_coords):
        ...
        for segment_number in range(number_of_segments):
            ...
            if (
                abs(segment_start_point_y - segment_end_point_y)
                < self.window.design.get_grid_size() / 10  # Horizontal part of the wire ...
                and abs(self.event_y - segment_start_point_y)
                <= distance_limit  # ... and the event is in the same y-coordinate range.
                and (
                    (segment_start_point_x - distance_limit)
                    <= self.event_x
                    <= (segment_end_point_x + distance_limit)  # ... and event is inside the x-range
                    or (segment_start_point_x + distance_limit)
                    >= self.event_x
                    >= (segment_end_point_x - distance_limit)
                )
            ):
                ...
                break
            if (
                abs(segment_start_point_x - segment_end_point_x)
                < self.window.design.get_grid_size() / 10  # Vertical part of the wire ...
                and abs(self.event_x - segment_start_point_x)
                <= distance_limit  # ... and the event is in the same x-coordinate range.
                and (
                    (segment_start_point_y - distance_limit) <= self.event_y <= (segment_end_point_y + distance_limit)
                    or (segment_start_point_y + distance_limit)
                    >= self.event_y
                    >= (segment_end_point_y - distance_limit)
                )
            ):
                segment_to_move = segment_number
    ...
    def _move_end(self, x_coordinate_to_change, y_coordinate_to_change, how_to_move_the_signal_name):
        ...
        if not (
            abs(coords[0] - coords[-2]) < self.window.design.get_grid_size() / 10
            and abs(coords[1] - coords[-1]) < self.window.design.get_grid_size() / 10
        ):  # Wires with length 0 or wire-circles are not allowed.
    ...
    def _move_modified_coordinates_to_the_grid(self, coords, x_coordinate_to_change, y_coordinate_to_change):
        ...
        remainder_x = (
            new_x % self.window.design.get_grid_size()
        )  # "%" always returns a remainder which has the same sign as the denominator.
        remainder_y = (
            new_y % self.window.design.get_grid_size()
        )  # This means: remainder_x, remainder_y are always positive.
