...
class InterfaceInsertion:
    ...
    def __draw_symbol_once_at_event_location(self, event, points):
        ...
        self.func_id_motion = self.diagram_tab.canvas.bind(
            "<Motion>", lambda event: self.__move_to(event, references_to_connected_wires)
        )
    ...
    def __draw(self, location_x, location_y, points):
        ...
        self.canvas_id = self.diagram_tab.canvas.create_polygon(
            points_moved, fill="violet", activefill="red", outline="black", tags=("layer2", "schematic-element")
        )
    ...
    def __move_start(self, event):
        ...
        list_overlapping_at_any_place = self.diagram_tab.canvas.find_overlapping(
        ...
        for canvas_id in list_overlapping_at_any_place:
            if self.diagram_tab.canvas.type(canvas_id) == "line" and "grid_line" not in self.diagram_tab.canvas.gettags(
                ...
                if (
                    line_coords[0] >= self.symbol_coords[0] - 0.1 * self.window.design.get_grid_size()
                    and line_coords[0] <= self.symbol_coords[0] + 0.1 * self.window.design.get_grid_size()
                    and line_coords[1] >= self.symbol_coords[1] - 0.1 * self.window.design.get_grid_size()
                    and line_coords[1] <= self.symbol_coords[1] + 0.1 * self.window.design.get_grid_size()
                ) or (
                    line_coords[-2] >= self.symbol_coords[0] - 0.1 * self.window.design.get_grid_size()
                    and line_coords[-2] <= self.symbol_coords[0] + 0.1 * self.window.design.get_grid_size()
                    and line_coords[-1] >= self.symbol_coords[1] - 0.1 * self.window.design.get_grid_size()
                    and line_coords[-1] <= self.symbol_coords[1] + 0.1 * self.window.design.get_grid_size()
                ):
        ...
        self.func_id_motion = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Motion>", lambda event: self.__move_to(event, references_to_connected_wires)
        )
        self.func_id_button_release = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<ButtonRelease-1>", lambda event: self.__move_end(references_to_connected_wires)
        )
    ...
    def __move_to(self, event, references_to_connected_wires):
        ...
        for reference in references_to_connected_wires:
            ...
            reference_to_wire.move_wire_end_point(
                "not move end", moved_point, new_event_x - self.event_x, new_event_y - self.event_y
            )
    ...
    def __add_bindings_to_symbol(self):
        ...
        self.sym_bind_funcid_dbutton = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Double-Button-1>", lambda event: self.__rotate(by_mouse=True)
        )
        self.sym_bind_funcid_rbutton = self.diagram_tab.canvas.tag_bind(
    ...
    def __create_bindings_for_interface_insertion_at_canvas(self, points):
        self.func_id_motion = self.diagram_tab.canvas.bind(
            "<Motion>", lambda event: self.__draw_symbol_once_at_event_location(event, points)
        )
        self.func_id_button = self.diagram_tab.canvas.bind(
            "<Button-1>", lambda event: self.__end_inserting(event, points)
        )
