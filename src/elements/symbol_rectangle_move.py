...
class RectangleMove:
        ...
        for canvas_id in canvas_ids:
            ...
            if canvas_type == "polygon":
                ...
                if "generic-map" in self.diagram_tab.canvas.gettags(canvas_id):
                    ...
                    self.diagram_tab.canvas.itemcget(canvas_id, "text").startswith(
                        self.symbol_definition["entity_name"]["name"] + "."
                    )  # VHDL
                    or self.diagram_tab.canvas.itemcget(canvas_id, "text")
                    == self.symbol_definition["entity_name"]["name"]
                ):  # Verilog
                    dictionary_of_symbol_items_to_move["entity_name"].append(canvas_id)
                elif self.symbol_definition["instance_name"]["name"] == self.diagram_tab.canvas.itemcget(
                    canvas_id, "text"
                ):
    ...
    def __get_minimal_rectangle(self, dictionary_of_symbol_items_to_move, touching_point, rectangle_coords):
        ...
        for kind in dictionary_of_symbol_items_to_move:
            if kind in ["polygons_left", "polygons_right"]:
                ...
                for canvas_id in (
                    canvas_id_list
                ):  # self.diagram_tab.canvas.coords(canvas_id)[0:2] is the polygon connection-point lying at the grid.
                    if (
                        self.diagram_tab.canvas.coords(canvas_id)[1]
                        < minimal_rectangle[1] + 0.5 * self.window.design.get_grid_size()
                    ):
                        minimal_rectangle[1] = (
                            self.diagram_tab.canvas.coords(canvas_id)[1] - 0.5 * self.window.design.get_grid_size()
                        )
                    if (
                        self.diagram_tab.canvas.coords(canvas_id)[1]
                        > minimal_rectangle[3] - 0.5 * self.window.design.get_grid_size()
                    ):
                        minimal_rectangle[3] = (
                            self.diagram_tab.canvas.coords(canvas_id)[1] + 0.5 * self.window.design.get_grid_size()
                        )
        for kind in dictionary_of_symbol_items_to_move:
            if kind in ["polygons_top", "polygons_bottom"]:
                ...
                for canvas_id in (
                    ...
                    if (
                        self.diagram_tab.canvas.coords(canvas_id)[0]
                        < minimal_rectangle[0] - 0.5 * self.window.design.get_grid_size()
                    ):
                        minimal_rectangle[0] = (
                            self.diagram_tab.canvas.coords(canvas_id)[0] + 0.5 * self.window.design.get_grid_size()
                        )
                    if (
                        self.diagram_tab.canvas.coords(canvas_id)[0]
                        > minimal_rectangle[2] + 0.5 * self.window.design.get_grid_size()
                    ):
                        minimal_rectangle[2] = (
                            self.diagram_tab.canvas.coords(canvas_id)[0] - 0.5 * self.window.design.get_grid_size()
                        )
    ...
    def __get_references_to_connected_wires(self, rectangle_coords):
        ...
        for line_points_dict in list_of_overlapping_line_points_dict:
            ...
            if touching_line_coords[0] > rectangle_coords[2]:
                references_to_connected_wires["right"].append(
                    [self.diagram_tab.design.get_references([canvas_id])[0], moved_point]
                )
            if touching_line_coords[1] < rectangle_coords[1]:
                references_to_connected_wires["top"].append(
                    [self.diagram_tab.design.get_references([canvas_id])[0], moved_point]
                )
            if touching_line_coords[1] > rectangle_coords[3]:
                references_to_connected_wires["bottom"].append(
                    [self.diagram_tab.design.get_references([canvas_id])[0], moved_point]
                )
        ...
        if rectangle_was_moved:
            for rectangle_add_on in dictionary_of_symbol_items_to_move:
                for canvas_id in dictionary_of_symbol_items_to_move[rectangle_add_on]:
                    self.diagram_tab.canvas.move(
                        canvas_id,
                        delta_x * delta_mask_dict[rectangle_add_on][0],
                        delta_y * delta_mask_dict[rectangle_add_on][1],
                    )
            for rectangle_side in references_to_connected_wires:
                ...
                for entry in references_to_connected_wires[rectangle_side]:
                    ...
                    wire_reference.move_wire_end_point(
    ...
    def __move_end(
        ...
        self.diagram_tab.canvas.tag_unbind(
            self.symbol_definition["object_tag"], "<ButtonRelease-1>", self.func_id_button_release
        )
