...
class SymbolUpdateInfos:
    """This class is used for updating the infos of a instance."""

    def __init__(
        self, root, window, diagram_tab, symbol, symbol_define_ref, update_generics, update_by_reading_from_other_file
    ):
        self.root = root
        self.window = window
        ...
        if instance_updated is None:
            return
        symbol_definition_upd = self._create_new_symbol_definition_from_source_file(instance_updated)
        generic_map_new = self._get_generic_map_from_new_symbol_definition(symbol_definition_upd)
        ...
        if symbol_definition_upd["additional_files"] != []:
            ...
            # If it is updated by a HDL-file, then the additional files from the symbol properties
            # must be left unchanged.
            # If it is updated by a HFE/HSE-file, then the additional files from the symbol properties
    ...
    def _create_new_symbol_definition_from_source_file(self, instance_updated):
    ...
    def _get_generic_map_from_new_symbol_definition(self, symbol_definition_upd):
