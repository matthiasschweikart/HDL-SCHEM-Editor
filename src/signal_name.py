...
class SignalName:
    ...
    def __add_bindings_to_signal_name(self):
        self.sym_bind_funcid_button1 = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Button-1>", self.__move_start_signal_name
        )
        self.sym_bind_funcid_dbutton1 = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Double-Button-1>", lambda event: self.__edit_signal_name()
        )
        self.sym_bind_funcid_button3 = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<ButtonRelease-3>", lambda event: self.__rotate_after_idle()
        )
        self.sym_bind_funcid_enter = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<Enter>", lambda event: self.__at_enter()
        )
    ...
    def __move_start_signal_name(self, event):
        ...
        self.func_id_button_release = self.diagram_tab.canvas.tag_bind(
            self.canvas_id, "<ButtonRelease-1>", self.__move_end
        )
    ...
    def __at_leave(self):
        ...
        self.diagram_tab.canvas.itemconfigure(
            self.canvas_id,
            text=self.__get_part_to_show_from_declaration(self.declaration),
            font=("Courier", self.design.get_font_size()),
        )
        self.__unhighlight()
    ...
    def __process_new_declaration(self, new_declaration, all_signals):
        ...
        if new_signal_name == old_signal_name:  # Only the other properties of wire (or net) are changed.
            ...
            self.__propagate_all_but_subrange_to_other_wires_with_old_signal_name(
            ...
            if (
                ...
                existing_signal_type, existing_init, existing_comment = (
                    self.__get_type_and_init_and_comment_from_a_wire_which_already_has_the_new_name(
    ...
    def __get_type_and_init_and_comment_from_a_wire_which_already_has_the_new_name(
    ...
    def __propagate_all_but_subrange_to_other_wires_with_old_signal_name(
    ...
    def set_declaration_if_signal_names_differ(
    ...
    def change_declaration(self, new_declaration):
        ...
        self.diagram_tab.canvas.itemconfigure(
            self.canvas_id, text=self.__get_part_to_show_from_declaration(new_declaration)
        )
