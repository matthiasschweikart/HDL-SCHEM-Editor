...
class Symbol:
    def __init__(
        ...
        self.symbol_definition["rectangle"]["canvas_id"] = self.diagram_tab.canvas.create_rectangle(
            *rectangle_coords,
            fill=symbol_color,
            activefill="red",
        ...
        if self.symbol_definition["architecture_name"] != "":
            entity_name = (
                self.symbol_definition["entity_name"]["name"] + "." + self.symbol_definition["architecture_name"]
            )
        else:
            entity_name = self.symbol_definition["entity_name"]["name"]
        self.symbol_definition["entity_name"]["canvas_id"] = self.diagram_tab.canvas.create_text(
            *self.symbol_definition["entity_name"]["coords"],
        ...
        self.symbol_definition["instance_name"]["canvas_id"] = self.diagram_tab.canvas.create_text(
            *self.symbol_definition["instance_name"]["coords"],
            font=("Courier", self.window.design.get_font_size()),
        ...
        if "generic_block" in self.symbol_definition:
            ...
            self.symbol_definition["generic_block"]["canvas_id"] = self.diagram_tab.canvas.create_text(
                *self.symbol_definition["generic_block"]["coords"],
                font=("Courier", self.window.design.get_font_size()),
                text=generic_map,
                anchor="sw",
                tags=(
        ...
        for port_entry in self.symbol_definition["port_list"]:
            ...
            port_entry["canvas_id"] = self.diagram_tab.canvas.create_polygon(
                *polygon_coords,
                outline="black",
                fill=symbol_color,
                activefill="red",
            ...
            port_entry["canvas_id_text"] = self.diagram_tab.canvas.create_text(
                polygon_coords[4] + text_delta_x,
                polygon_coords[5] + text_delta_y,
                font=("Courier", self.window.design.get_font_size()),
                text=combined_port_name,
                anchor=text_anchor,
                angle=text_angle,
                tags=(self.symbol_definition["object_tag"], "instance-text", "layer3", "schematic-element"),
            )
    ...
    def __update_coords_info_in_symbol_definition(self):
        ...
        if "port_list" in self.symbol_definition:
            for index, port_entry in enumerate(self.symbol_definition["port_list"]):
                self.symbol_definition["port_list"][index]["coords"] = self.diagram_tab.canvas.coords(
                    port_entry["canvas_id"]
                )
        if "generic_block" in self.symbol_definition:
            self.symbol_definition["generic_block"]["coords"] = self.diagram_tab.canvas.coords(
    ...
    def __add_bindings_to_symbol(self):
        self.sym_bind_funcid_button = self.diagram_tab.canvas.tag_bind(
            self.symbol_definition["rectangle"]["canvas_id"],
            "<Button-1>",
            lambda event: symbol_rectangle_move.RectangleMove(
                event, self.window, self.diagram_tab, self, self.symbol_definition
            ),
        )
        ...
        for port_definition in self.symbol_definition["port_list"]:
            ...
            self.sym_bind_funcid_port_show[port_definition["canvas_id_text"]] = self.diagram_tab.canvas.tag_bind(
                port_definition["canvas_id_text"],
                "<Enter>",
                lambda event, canvas_id_text=port_definition["canvas_id_text"]: self.show_port_type(canvas_id_text),
            )
            self.sym_bind_funcid_port_hide[port_definition["canvas_id_text"]] = self.diagram_tab.canvas.tag_bind(
                port_definition["canvas_id_text"],
                "<Leave>",
        ...
        self.sym_bind_funcid_edit_in = self.diagram_tab.canvas.tag_bind(
            self.symbol_definition["instance_name"]["canvas_id"],
            "<Double-Button-1>",
            lambda event: edit_line.EditLine(
                self.window.design, self.diagram_tab, self.symbol_definition["instance_name"]["canvas_id"], self
            ),
        )
        if "generic_block" in self.symbol_definition:
            self.sym_bind_funcid_edit_gb = self.diagram_tab.canvas.tag_bind(
                self.symbol_definition["generic_block"]["canvas_id"],
                "<Double-Button-1>",
                lambda event: edit_text.EditText(
                    "generic_block",
                    self.window,
                    self.diagram_tab,
                    self.symbol_definition["generic_block"]["canvas_id"],
        ...
        for entry in self.symbol_definition["port_list"]:
            if entry["canvas_id_text"] == canvas_id_text:
                ...
                self.background_rectangle = self.diagram_tab.canvas.create_rectangle(
                    self.diagram_tab.canvas.bbox(canvas_id_text), fill="white"
                )
    ...
    def hide_port_type(self, canvas_id_text):
        ...
        if self.background_rectangle is not None:
            ...
            self.diagram_tab.canvas.itemconfigure(
                canvas_id_text, text=self.original_text, font=("Courier", self.window.design.get_font_size())
            )
            self.background_rectangle = None
    ...
    def __show_symbol_info(self, canvas_id):
        ...
        if "instance-name" in tags:  # Get also the entity name.
            ...
            text = self.original_text + "\n"
            text += (
                self.diagram_tab.canvas.itemcget(self.symbol_definition["instance_name"]["canvas_id"], "text") + "\n"
            )
        text += self.diagram_tab.design.get_language()
        self.diagram_tab.canvas.itemconfigure(canvas_id, text=text, font=("Courier", 10))
        self.background_rectangle = self.diagram_tab.canvas.create_rectangle(
            self.diagram_tab.canvas.bbox(canvas_id), fill="white"
        )
    ...
    def __remove_bindings_from_symbol(self):
        ...
        self.diagram_tab.canvas.tag_unbind(
            self.symbol_definition["rectangle"]["canvas_id"], "<Double-Button-1>", self.sym_bind_funcid_dbutton
        )
        self.diagram_tab.canvas.tag_unbind(
            self.symbol_definition["rectangle"]["canvas_id"], "<Enter>", self.sym_bind_funcid_enter
        )
        self.diagram_tab.canvas.tag_unbind(
            self.symbol_definition["rectangle"]["canvas_id"], "<Leave>", self.sym_bind_funcid_leave
        )
        self.diagram_tab.canvas.tag_unbind(
            self.symbol_definition["entity_name"]["canvas_id"], "<Enter>", self.sym_bind_funcid_show1
        )
        self.diagram_tab.canvas.tag_unbind(
        ...
        self.diagram_tab.canvas.tag_unbind(
            self.symbol_definition["instance_name"]["canvas_id"], "<Enter>", self.sym_bind_funcid_show2
        )
        self.diagram_tab.canvas.tag_unbind(
            self.symbol_definition["instance_name"]["canvas_id"], "<Leave>", self.sym_bind_funcid_hide2
        )
        for port_definition in self.symbol_definition["port_list"]:
            self.diagram_tab.canvas.tag_unbind(
                port_definition["canvas_id"], "<Button-1>", self.sym_bind_funcid_polygons[port_definition["canvas_id"]]
            )
            self.diagram_tab.canvas.tag_unbind(
                port_definition["canvas_id_text"],
                "<Enter>",
                self.sym_bind_funcid_port_show[port_definition["canvas_id_text"]],
            )
            self.diagram_tab.canvas.tag_unbind(
                port_definition["canvas_id_text"],
                "<Leave>",
                self.sym_bind_funcid_port_hide[port_definition["canvas_id_text"]],
            )
        if "generic_block" in self.symbol_definition:
            self.diagram_tab.canvas.tag_unbind(
                self.symbol_definition["generic_block"]["canvas_id"], "<Double-Button-1>", self.sym_bind_funcid_edit_gb
            )
        self.sym_bind_funcid_button = None
        self.sym_bind_funcid_dbutton = None
        self.sym_bind_funcid_enter = None
        ...
        for port_entry in self.symbol_definition["port_list"]:
            ...
            if self.__port_is_open(polygon_coords):
                ...
                if polygon_coords[0] < self.symbol_definition["rectangle"]["coords"][0]:  # left
                    position = "left"
                    wire_coords = [
                        polygon_coords[0] - 8 * self.window.design.get_grid_size(),
                        polygon_coords[1],
                        polygon_coords[0],
                        polygon_coords[1],
                    ]
                    ...
                    wire_coords = [
                        ...
                        polygon_coords[1],
                        polygon_coords[0],
                        polygon_coords[1] - 8 * self.window.design.get_grid_size(),
                    ]
                ...
                if port_name.endswith("_i") or port_name.endswith("_o") or port_name.endswith("_io"):
                    if not ask_for_each_suffix:
                        ...
                        remove_this_port_name_suffix = messagebox.askquestion(
                            'Found suffix "_i", "_o", "_io":', "Remove suffix in " + port_name + "?", default="no"
                        )
                ...
                wire_ref = wire_insertion.Wire(
                    self.root,
                    self.window,
                    self.diagram_tab,  # push_design_to_stack=False,
                    coords=wire_coords,
                    tags="adding_signal_stubs",
                    arrow="none",
                    width=width,
                )
                ...
                signal_name.SignalName(
                    ...
                    coords=[wire_coords[0] + signal_name_delta_x, wire_coords[1] + signal_name_delta_y],
                    angle=signal_name_angle,
                    wire_tag=wire_tag,
    ...
    def __create_vhdl_signal_declaration(self, port_declaration):
        if self.symbol_definition["language"] == "VHDL":
            ...
            if "[" in port_declaration:
                ...
                signal_declaration = (
                    port_declaration_list[-1]
                    + " : "
                    + "std_logic_vector("
                    + bounds[0]
                    + port_range_direction
                    + bounds[1]
                    + ")"
                )
            else:
    ...
    def __get_port_direction_from_port_declaration(self, port_declaration):
        if self.symbol_definition["language"] == "VHDL":
            ...
            port_declaration_without_comment = re.sub(
                r"//.*", "", port_declaration
            )  # remove comments at the end of the string.
    ...
    def __open_source_code_after_idle(self):
        ...
        self.window.after_idle(
            Symbol.open_source_code,
            self.root,
            self.window,
            self.symbol_definition["filename"],
            self.symbol_definition["architecture_name"],
        )
        ...
        for key in update_list:
            if key == "library":  # key will be used by symbol_properties.py and by symbol_update_infos.py.
                ...
                symbol_update_infos.SymbolUpdateInfos(
                    ...
                    symbol_define_ref,
                    update_generics=True,
                    update_by_reading_from_other_file=True,
                )
                ...
                if self.symbol_definition["architecture_name"] != "":
                    entity_name = (
                        self.symbol_definition["entity_name"]["name"]
                        + "."
                        + self.symbol_definition["architecture_name"]
                    )
                else:
                    entity_name = self.symbol_definition["entity_name"]["name"]
                self.diagram_tab.canvas.itemconfigure(
                    self.symbol_definition["entity_name"]["canvas_id"], text=entity_name
                )
                ...
                self.diagram_tab.canvas.itemconfigure(
                    self.symbol_definition["generic_block"]["canvas_id"], text=generic_map
                )
    ...
    def get_pin_list(cls, symbol_definition):
        ...
        for port_list_entry in symbol_definition["port_list"]:
            ...
            list_entry = {
                "type": entity_call,  # The key "type" is used, because pin_list is combined with a second dictionary
                "instance_name": symbol_definition["instance_name"][
                    "name"
                ],  # which already has this key, see hdl_generate.HdlGenerate.create _declarations().
                "architecture_name": symbol_definition[
                    "architecture_name"
                ],  # This name is used when highlighting through hierarchy is used.
    ...
    def get_embedded_configuration(cls, symbol_definition):
        if symbol_definition["configuration"]["config_statement"] == "Embedded":
            ...
            config_statement += (
                " use entity "
                + symbol_definition["configuration"]["library"]
                + "."
                + symbol_definition["entity_name"]["name"]
            )
            if symbol_definition["architecture_name"] != "":
    ...
    def get_symbol_definition_shifted(cls, symbol_definition, offset):
        ...
        symbol_definition["instance_name"]["coords"] = [
            coord + offset for coord in symbol_definition["instance_name"]["coords"]
        ]
        symbol_definition["rectangle"]["coords"] = [
            coord + offset for coord in symbol_definition["rectangle"]["coords"]
        ]
        symbol_definition["generic_block"]["coords"] = [
            coord + offset for coord in symbol_definition["generic_block"]["coords"]
        ]
    ...
    def open_source_code(cls, root, window, path_name, arch_name):
        for open_window in schematic_window.SchematicWindow.open_window_dict:
            if open_window.design.get_path_name() == path_name:
                if (
                    open_window.design.get_architecture_name() != arch_name and arch_name != ""
                ):  # Some old Verilog-designs may have the arch-name "".
        ...
        if path_name.endswith(".hse"):
            ...
            if path_name.endswith(".hfe"):
                ...
                if command == "" or command.isspace():
                    messagebox.showerror(
                        ...
                        'Cannot open source code of symbol because no "edit command" is defined in the control tab.',
                    )
                    return
            ...
            try:
                ...
                messagebox.showerror(
                    "Error in HDL-SCHEM-Editor", "Error when running " + command + ' "' + path_name + '"'
                )
    ...
    def try_to_replace_not_found_name_by_correct_one(cls, window, path_name):
        ...
        if os.path.isfile(new_path_name):
            use_new_filename = messagebox.askquestion(
