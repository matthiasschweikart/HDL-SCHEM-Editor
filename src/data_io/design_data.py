...
import constants
from codegen import hdl_generate
...
from gui import notebook_diagram_tab
...
class DesignData:
    ...
    def __init__(self, root, schematic_window):
        ...
        self.change_stack.append(
            self.create_design_dictionary()
        )  # Put an empty design at the stack (needed for the undo of the first action).
    ...
    def store_interface_in_canvas_dictionary(
        ...
        )  # Checks window_title, must be placed after "self.update_window_title(written=False)"
        if self.debug_stack:
    ...
    def create_design_dictionary(self):
        ...
        input_decl, output_decl, inout_decl, _, _ = hdl_generate.GenerateHDL.create_declarations(
            self.language, self.grid_size, connector_location_list, wire_location_list
        )
        design_dictionary = {}
        ...
        design_dictionary["additional_sources"] = (
            self.additional_sources
        )  # Contains a string with a list of comma separated file names.
    ...
    def create_schematic_elements_dictionary(self):  # Used by hdl_generate_sort_elements.SortElements
        ...
        for canvas_id, element_description_list in self.canvas_dictionary.items():
            if element_description_list[1] == "instance":
                ...
                new_hdl_element_entry["prio"] = symbol_instance.Symbol.get_priority_from_symbol_definition(
                    element_description_list[2]
                )
                ...
                new_hdl_element_entry["prio"] = block_insertion.Block.get_priority_from_text(
                    element_description_list[4]
                )
                new_hdl_element_entry["coords"] = []
                hdl_element_dict[canvas_id] = new_hdl_element_entry
                ...
                new_hdl_element_entry["prio"] = generate_frame.GenerateFrame.get_priority_from_generate_definition(
                    element_description_list[2]
                )
                new_hdl_element_entry["coords"] = (
                    generate_frame.GenerateFrame.get_rectangle_coords_from_generate_definition(
                        element_description_list[2]
                    )
                )
                hdl_element_dict[canvas_id] = new_hdl_element_entry
    ...
    def get_references(self, canvas_ids=None):
        ...
        if canvas_ids is None:
            ...
            for canvas_id in canvas_ids:
                if (
                    canvas_id in self.canvas_dictionary
                ):  # Instances (and also other objects) do not store all their canvas-IDs in the dictionary.
                    ref_list.append(
                        self.canvas_dictionary[canvas_id][0]
                    )  # Return the references to the objects, which are selected by canvas_ids.
        return ref_list
        ...
        if push_design_to_stack:
            ...
            self.visible_center_point = [
                (
                    ...
                    + self.window.notebook_top.diagram_tab.canvas.canvasx(
                        self.window.notebook_top.diagram_tab.canvas.winfo_width()
                    )
                )
                / 2,
                (
                    self.window.notebook_top.diagram_tab.canvas.canvasy(0)
                    + self.window.notebook_top.diagram_tab.canvas.canvasy(
                        self.window.notebook_top.diagram_tab.canvas.winfo_height()
                    )
                )
                / 2,
            ]
        ...
        for canvas_id in canvas_ids:
            if canvas_id in window.design.get_canvas_ids_of_elements():
                if window.design.get_schematic_element_type_of(canvas_id) == "input":
                    ref = interface_input.Input(
                        self.window,
                        self.window.notebook_top.diagram_tab,
                        dummy,
                    ...
                    ref = wire_insertion.Wire(
                        self.root,
                        self.window,
                        self.window.notebook_top.diagram_tab,  # push_design_to_stack=False,
                    ...
                    if (
                        wire_tag in object_tag_dict
                    ):  # Then not only a signal name was copied, but also the corresponding wire.
                        tag = object_tag_dict[window.design.get_tag_of_signal_name(canvas_id)]
                        ref = signal_name.SignalName(
                            self,
                            self.window.notebook_top.diagram_tab,  # push_design_to_stack=False,
                    ...
                    generate_definition_copy = json.loads(
                        json.dumps(window.design.get_generate_definition_of(canvas_id))
                    )
                    ...
                    ref = generate_frame.GenerateFrame(
                        self.root, self.window, self.window.notebook_top.diagram_tab, generate_definition_copy
                    )
                    references_of_copies.append(ref)
    ...
    def __move_copies(self, move_copies_under_the_cursor):
        ...
        if move_copies_under_the_cursor:
            canvas_position_x = self.window.notebook_top.diagram_tab.canvas.canvasx(
                self.window.notebook_top.diagram_tab.canvas.winfo_pointerx()
                - self.window.notebook_top.diagram_tab.canvas.winfo_rootx()
            )
            canvas_position_y = self.window.notebook_top.diagram_tab.canvas.canvasy(
                self.window.notebook_top.diagram_tab.canvas.winfo_pointery()
