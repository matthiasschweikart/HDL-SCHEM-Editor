...
class FileRead:
    """This class reads the schematic from a JSON file and updates the schematic window."""
            ...
            for open_window, open_file in window.__class__.open_window_dict.items():
                if filename == open_file:
                    ...
                    if window != open_window:
                        ...
                        # As there is already a window with the new content, this must used,
                        # but the existing window must be closed.
                        window.close_this_window()
                    return
        if (
            filename
            != ""  # filename is equal "", when the user has pressed abort or used the Escape-Key at askopenfilename().
            and filename  # Contrary to documention, instead of "" an empty tuple () is returned in case of abort.
        ):
            ...
            try:
                ...
                # As interface and internal-tab must be displayed to position sash correctly,
                # this is done later: window.notebook_top.show_tab("Diagram")
                ...
                pass  # messagebox.showerror("Error ", "File " + filename + " could not be found at read.")
