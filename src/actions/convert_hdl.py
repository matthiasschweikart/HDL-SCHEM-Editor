...
class ConvertHdl:
    ...
    def __init__(self, window, language):
        if language == "VHDL":
            hdl_file_name = askopenfilename(
                filetypes=(("VHDL files", "*.vhd"), ("all files", "*.*")), title="Select HDL file to convert:"
            )
        else:
            hdl_file_name = askopenfilename(
                filetypes=(("Verilog files", "*.v"), ("all files", "*.*")), title="Select HDL file to convert:"
            )
        if hdl_file_name != "":
    ...
    def __initialize_design_dictionary(self, window, language):
        ...
        design_dictionary["text_dictionary"] = {
            "interface_packages": "",
            "interface_generics": "",
            "internals_packages": "",
    ...
    def __fill_dictionary_with_entity_generics(self, design_dictionary, hdl_parsed, language):
        ...
        for index, generic_interface_name in enumerate(generics_interface_names):
            if language == "VHDL":
                design_dictionary["text_dictionary"]["interface_generics"] += (
                    generic_interface_name.strip()
                    + " : "
                    + generics_interface_types[index].strip()
                    + generics_interface_ranges[index].strip()
                    + init_assign
                ...
                design_dictionary["text_dictionary"]["interface_generics"] += (
                    generic_interface_name.strip() + init_assign + generics_interface_init[index].strip() + ",\n"
                )
        return
    ...
    def __fill_dictionary_with_architecture_declarations(self, design_dictionary, hdl_parsed, language):
        if language == "VHDL":
            ...
            for architecture_library_name in architecture_library_names:
                design_dictionary["text_dictionary"]["internals_packages"] += (
                    "library " + architecture_library_name + ";\n"
                )
            for architecture_package_name in architecture_package_names:
                design_dictionary["text_dictionary"]["internals_packages"] += (
                    "use " + architecture_package_name + ".all;\n"
                )
    ...
    def __fill_canvas_dictionary_with_ports(self, design_dictionary, hdl_parsed, language):
        ...
        for index, port_interface_name in enumerate(port_interface_names):
            ...
            if port_interface_direction[index] in ["in", "input"]:
                ...
                connector_x_coord = wire_length
            port_interface_ranges[index] = re.sub(
                r"\n", " ", port_interface_ranges[index]
            )  # range may be separated into several lines.
            port_interface_ranges[index] = re.sub(
                r"\s+", " ", port_interface_ranges[index]
            )  # Convert multiple blanks into one blank.
            if language == "VHDL":
                port_interface_init[index] = re.sub(
                    r"\n", " ", port_interface_init[index]
                )  # init may be separated into several lines.
                port_interface_init[index] = re.sub(
                    r"\s+", " ", port_interface_init[index]
                )  # Convert multiple blanks into one blank.
            ...
            design_dictionary["canvas_dictionary"][canvas_dict_key] = [
                "empty",
                connector_type,
                [connector_x_coord, index * design_dictionary["grid_size"]],
                0,
            ]
            canvas_dict_key += 1
            if language == "VHDL":
                design_dictionary["canvas_dictionary"][canvas_dict_key] = [
                    "empty",
                    "signal-name",
                    [0, index * design_dictionary["grid_size"]],
                    0,
                    port_interface_name
                    + " : "
                ...
                if connector_type == "input":
                    port_interface_types[index] = (
                        "wire " + port_interface_types[index]
                    )  # needed for verilog inputs, where neither "reg" nor "wire" is specified
                design_dictionary["canvas_dictionary"][canvas_dict_key] = [
                    "empty",
                    "signal-name",
                    [0, index * design_dictionary["grid_size"]],
                    0,
                    port_interface_types[index] + " " + port_interface_ranges[index] + port_interface_name,
                    "wire_" + str(index),
                ]
            canvas_dict_key += 1
            design_dictionary["canvas_dictionary"][canvas_dict_key] = [
                "empty",
                "wire",
                [0, index * design_dictionary["grid_size"], wire_length, index * design_dictionary["grid_size"]],
                ["wire_" + str(index), "layer2", "schematic-element"],
                "none",
                size,
            ]
            canvas_dict_key += 1
    ...
    def __fill_canvas_dictionary_with_block(self, design_dictionary, hdl_parsed, canvas_dict_key, index):
        ...
        if number_of_lines != 0:
            design_dictionary["canvas_dictionary"][canvas_dict_key] = [
                ...
                [
                    0,
                    index * design_dictionary["grid_size"],
                    1000,
                    (number_of_lines + index) * design_dictionary["grid_size"],
                ],
