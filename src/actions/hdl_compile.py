...
from codegen import hdl_create_file_list, hdl_generate_flipflop_stat, hdl_generate_functions
from gui import notebook_log_tab, notebook_top
...
class CompileHDL:
    def __init__(
        ...
        if (
            self.__change_directory_to_working_directory_is_not_possible()
            or self.__hdl_is_not_up_to_date()
            or self.__design_changes_are_not_saved()
        ):
            return
        ...
        if self.design.get_language() != "VHDL":
            ...
            if self.design.get_number_of_files() == 1:
                ...
                hdlfilename_architecture = (
                    self.design.get_generate_path_value()
                    + "/"
                    + self.design.get_module_name()
                    + "_"
                    + self.design.get_architecture_name()
                    + ".vhd"
                )
        return hdl_generate_functions.HdlGenerateFunctions.hdl_must_be_generated(
            self.design.get_path_name(), hdlfilename, hdlfilename_architecture, show_message=True
        )
    ...
    def __get_commands_as_list(self):
        if self.compile_through_hierarchy:
            ...
            message_for_error = (
                "The compile through hierarchy command is not specified.\nSpecify it in the Control-Tab."
            )
        else:
            compile_command = self.design.get_compile_cmd()
            message_for_error = (
                "The compile command for a single module is not specified.\nSpecify it in the Control-Tab."
            )
    ...
    def __put_header_in_message_tab(self):
        ...
        self.log_tab.insert_line_in_log(
            "Working Directory: " + self.window.design.get_working_directory() + "\n", state_after_insert="disabled"
        )
    ...
    def __run_commands(self, commands, flipflop_stat):
        ...
        self.log_tab.insert_line_in_log(
            "Finished user commands from Control-Tab after " + str(end_time - self.start_time) + ".\n",
            state_after_insert="disabled",
        )
    ...
    def __replace_variables(self, command_array):
        ...
        for index, _ in enumerate(command_array):
            ...
            command_array[index] = re.sub(
                r"\$hdl-file-list", "hdl_file_list_" + module_name + ".txt", command_array[index]
            )
        ...
        for flipflop_stat_list_line in flipflop_stat_list:
            ...
            if "no clocked signals found" in flipflop_stat_list_line:
                instance_name = re.sub(
                    ...
                    r"\1",
                    flipflop_stat_list_line,
                    flags=re.DOTALL,
                )
                signal_name = "no-clocked-signal-existent"
            else:
                instance_name = re.sub(
                    r".*flipflop_statistics for instance (.*) signal .*",
                    r"\1",
                    flipflop_stat_list_line,
                    flags=re.DOTALL,
                )
        ...
        for _, signal_name_dict in flipflop_dict.items():
            for _, signal_prop_dict in signal_name_dict.items():
                if "generate_condition" in signal_prop_dict and (
                    signal_prop_dict["generate_condition"] == "false"  # VHDL simulator
                    or signal_prop_dict["generate_condition"] == "0"
                ):  # Verilog simulator
        ...
        for _, signal_name_dict in flipflop_dict.items():
            for _, signal_prop_dict in signal_name_dict.items():
                if "factor" in signal_prop_dict:
                    signal_prop_dict["flipflops"] = str(
                        int(signal_prop_dict["flipflops"]) * int(signal_prop_dict["factor"])
                    )

    def __calculate_sum_of_flipflops(self, flipflop_dict):
        sum_of_flipflops = 0  # Sum of flipflops in complete design
        for _, signal_name_dict in flipflop_dict.items():
            if "no-clocked-signal-existent" not in signal_name_dict:
                for _, signal_prop_dict in signal_name_dict.items():
    ...
    def __create_table_for_log_tab(self, flipflop_dict, sum_of_flipflops):
            ...
            for instance_name in instance_names:
                ...
                for signal_name in flipflop_dict[instance_name]:
                    if signal_name != "no-clocked-signal-existent":
                        table.append(
                            "    | "
                            + " " * 4 * (indent_new - indent_init)
                            + instance_name_short
                            + " § "
                            + signal_name
                            + " # "
                            + flipflop_dict[instance_name][signal_name]["flipflops"]
                            + " %"
                        )
                    else:
                        table.append(
        ...
        table_for_log += (
            ...
            + "-" * (index_separator1 - 4)
            + "+"
            + "-" * (index_separator2 - index_separator1 - 1)
            + "+"
            + "-" * (index_separator3 - index_separator2)
            + "\n"
        )
        ...
        table_for_log += (
            " " * (index_separator2 - 19)
            + "Total flipflop sum:  "
            + " " * (index_separator3 - index_separator2 - len(str(sum_of_flipflops)) - 3)
            + str(sum_of_flipflops)
            + "\n"
        )
    ...
    def __put_table_into_messages_tab(self, table_for_log):
        ...
        if "/" in current_working_directory:
            ...
            current_working_directory += "\\"
        self.log_tab.insert_line_in_log(
            "File with all simulator messages regarding flipflop statistic:\n"
            + current_working_directory
            + current_module_name
            + "/"
