...
class NotebookDiagramTab:
    ...
    def __init__(
        ...
        canvas_height = (
            400  # Otherwise the canvas width would be bigger than the scroll_region, which would be a mistake.
        )
        ...
        self.canvas_scrollbar_hor = ttk.Scrollbar(
            self.canvas_frame, orient=tk.HORIZONTAL, cursor="arrow", style="Horizontal.TScrollbar"
        )
        ...
        self.canvas = tk.Canvas(
            ...
            width=canvas_width,
            scrollregion=self.canvas_visible_area,
            bg=self.root.schematic_background_color,
            xscrollcommand=self._scroll_in_x_direction,
            yscrollcommand=self._scroll_in_y_direction,
        )
        ...
        self.treeview_scrollbar_hor = ttk.Scrollbar(
            self.treeview_frame, orient=tk.HORIZONTAL, cursor="arrow", style="Horizontal.TScrollbar"
        )
        ...
        for column_name in self.window.hierarchytree.column_names:
            text = self.window.hierarchytree.column_name_of_column0 if column_name == "#0" else column_name
        ...
        self.treeview_scrollbar_ver.grid(
            row=0, column=1, sticky=(tk.N, tk.S)
        )  # The sticky argument extends the scrollbar, so that a "shift" is possible.
        self.treeview_scrollbar_hor.grid(
            row=1, column=0, sticky=(tk.W, tk.E)
        )  # The sticky argument extends the scrollbar, so that a "shift" is possible.
        self.treeview_scrollbar_ver["command"] = self.treeview.yview
        self.treeview_scrollbar_hor["command"] = self.treeview.xview
        self.treeview.bind(
            "<Double-Button-1>", lambda event: self.window.hierarchytree.open_design_in_new_window(event, self.treeview)
        )

        notebook.add(
            self.diagram_frame, sticky=tk.N + tk.E + tk.W + tk.S, text="Diagram"
        )  # As schematic_window.notebook_top.notebook does not yet exist, when the constructor is called,
        ...
        self.architecture_label = ttk.Label(
            self.architecture_frame, takefocus=False, text="Architecture:", justify=tk.CENTER
        )
        self.architecture_combobox = ttk.Combobox(
            self.architecture_frame, takefocus=False, state="readonly", values=self.architecture_list, justify=tk.CENTER
        )
        self.rename_architecture_button = ttk.Button(
            self.architecture_frame, takefocus=False, text="rename Architecture", style="NewState.TButton"
        )
        self.new_architecture_button = ttk.Button(
            self.architecture_frame, takefocus=False, text="new Architecture", style="NewState.TButton"
        )
        self.delete_architecture_button = ttk.Button(
            self.architecture_frame,
            takefocus=False,
            text="delete Architecture",
            style="NewState.TButton",
            state="disabled",
        )
        ...
        self.new_input_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Input", style="NewState.TButton"
        )
        self.new_output_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Output", style="NewTransition.TButton"
        )
        self.new_inout_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Inout", style="NewConnector.TButton"
        )
        self.new_wire_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Wire", style="ResetEntry.TButton"
        )
        self.new_bus_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Bus", style="ResetEntry.TButton"
        )
        self.new_block_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Block", style="DefaultStateActions.TButton"
        )
        self.new_instance_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Instance", style="DefaultStateActions.TButton"
        )
        self.new_generate_button = ttk.Button(
            self.new_buttons_frame, takefocus=False, text="new Generate", style="DefaultStateActions.TButton"
        )
        ...
        self.copy_button = ttk.Button(
            self.copy_paste_frame, text="Copy Selection (Ctrl-c)", state="disabled", command=self.copy
        )
        self.paste_button = ttk.Button(
            self.copy_paste_frame, text="Paste (Ctrl-v)", state="disabled", command=self.paste_from_button
        )
        ...
        self.view_all_button = ttk.Button(
            self.view_buttons_frame, takefocus=False, text="view all", style="View.TButton"
        )
        self.view_area_button = ttk.Button(
            self.view_buttons_frame, takefocus=False, text="view area", style="View.TButton"
        )
        self.view_last_button = ttk.Button(
            self.view_buttons_frame, takefocus=False, text="view last", style="View.TButton"
        )
        self.view_inst_button = ttk.Button(
            self.view_buttons_frame, takefocus=False, text="view instance", style="View.TButton"
        )
    ...
    def _scroll_in_y_direction(self, first, last):
        if last == "1.0":
            self.canvas_visible_area[3] = self.canvas_visible_area[3] + 0.025 * (
                self.canvas_visible_area[3] - self.canvas_visible_area[1]
            )
            self.canvas.configure(scrollregion=self.canvas_visible_area)
        elif first == "0.0":
            self.canvas_visible_area[1] = self.canvas_visible_area[1] - 0.025 * (
                self.canvas_visible_area[3] - self.canvas_visible_area[1]
            )
            self.canvas.configure(scrollregion=self.canvas_visible_area)
    ...
    def _scroll_in_x_direction(self, first, last):
        if last == "1.0":
            self.canvas_visible_area[2] = self.canvas_visible_area[2] + 0.002 * (
                self.canvas_visible_area[2] - self.canvas_visible_area[0]
            )
            self.canvas.configure(scrollregion=self.canvas_visible_area)
        elif first == "0.0":
            self.canvas_visible_area[0] = self.canvas_visible_area[0] - 0.002 * (
                self.canvas_visible_area[2] - self.canvas_visible_area[0]
            )
            self.canvas.configure(scrollregion=self.canvas_visible_area)
    ...
    def diagram_tab_is_shown(self):
        """Called when the diagram tab is shown."""
    ...
    def diagram_tab_is_hidden(self):
        """Called when the diagram tab is hidden."""
    ...
    def sort_layers(self):
        """Sort the layers of the canvas, so that the elements are displayed in the correct order."""
    ...
    def get_enclosed_elements(self, coords):
        """Get the canvas ids of the elements, which are enclosed by the rectangle with the given coordinates."""
        element_canvas_ids = []
        enclosed_element_list = self.canvas.find_enclosed(*coords)
        for canvas_id in enclosed_element_list:
            if canvas_id in self.design.get_canvas_ids_of_elements() and self.design.get_schematic_element_type_of(
                canvas_id
            ) in ["block", "instance", "generate_frame"]:
    ...
    def _add_overlapping_block_text_elements(self, enclosed_element_list, element_canvas_ids):
        for canvas_id in enclosed_element_list:
            if (
                self.canvas.type(canvas_id) == "rectangle"
                and self.design.get_schematic_element_type_of(canvas_id) == "block-rectangle"
            ):
                ...
                for single_canvas_id_of_object in list_of_canvas_ids_of_object:
                    if single_canvas_id_of_object != canvas_id and single_canvas_id_of_object not in element_canvas_ids:
    ...
    def copy(self):
        ...
        if references:
            ...
            messagebox.showerror(
                "Error in HDL-SCHEM-Editor", "Nothing selected. Select by dragging a rectangle with the mouse pointer."
            )
        ...
        # When copy was called by the Copy-button, focus changed to the button, which will not react to
        # a Control-v which might be the next user action:
    ...
    def _get_canvas_ids_of_all_items_of_the_selected(self, references):
        ...
        for reference in references:
            ...
            # Detect the integer object_tags of interface-connectors so that they are not
            # checked by ".endswith" in the next line.
            # Do not add signal-name-object-tags to this list, otherwise signal_names without wires could be copied:
            if isinstance(object_tag, int) or not object_tag.endswith("_signal_name"):
                all_object_tags.append(object_tag)
            else:  # Only signal_names remain here.
                signal_name_object_tags.append(
                    object_tag
                )  # This list is needed for later adding to the "selected"-tag, so that _undo selection() can work.
        ...
        for object_tag in (
            signal_name_object_tags
        ):  # These canvas items shall not be included in NotebookDiagramTab.selected_canvas_ids_for_copy.
    ...
    def _copy_selected_to_clipboard(self, selected_canvas_ids_for_copy):
        ...
        references_of_copies = NotebookDiagramTab.clipboard_window.design.insert_copies_from(
            self.window, selected_canvas_ids_for_copy, move_copies_under_the_cursor=False
        )
    ...
    def paste_from_button(self):
        """Paste the elements from the clipboard to the canvas. Called by the paste-button."""
        self.canvas.focus_set()
        self.canvas.event_generate("<Control-v>", when="now")  # event coordinates are not defined.
        # The event "<Control-v>" calls paste() and paste() draws the new elements and calls at
        # last _move_selection_start(). But as the mouse pointer is still located at the button "Paste",
        # the "<Motion>" event must be deactivated for the time, when the user moves the mouse pointer
        # from the button to the selected objects. When the user clicks at the selected objects, in order
        # to move them, _move_selection_start() is called a second time and "Motion" is bound again:
        self.canvas.unbind("<Motion>", self.funcid_motion)
        self.funcid_motion = None
        # The binding of ButtonRelease-1 must also be removed, because when the user clicks at the pasted
        # objects in order to move them, _move_selection-start is called and decides
        # by self.funcid_button1_release if Motion must be bound again:
        self.canvas.unbind("<ButtonRelease-1>", self.funcid_button1_release)
        self.funcid_button1_release = None

    def paste(self, event):
        """Paste the elements from the clipboard to the canvas. Called by the event Control-v."""
        self.window.event_generate(
            "<FocusIn>", when="now"
        )  # get the focus back from the clipboard and bind the accellerators to this window.
        move_copies_under_the_cursor = bool(
            event.send_event
        )  # Check if the event "control-v" was created by keyboard (instead by paste-button)
        self._unselect_elements()
        if NotebookDiagramTab.clipboard_window is not None:
            references_of_copies = self.window.design.insert_copies_from(
                NotebookDiagramTab.clipboard_window,
                NotebookDiagramTab.selected_canvas_ids_for_copy,
                move_copies_under_the_cursor,
            )
            if references_of_copies:
                all_object_tags = []
                ...
                for object_tag in all_object_tags:
                    self.canvas.addtag_withtag("selected", object_tag)
                # A copy of a symbol is not yet stored in the design_data.canvas_dictionary, so the references must
                # be taken from here. When Control-v was pressed then the copy shall react to mouse-movements and
                # shall not need a pick up by Button-1:
                self._move_selection_start(event, references=references_of_copies)
                # When the paste button was pressed, then moving the selection has to start by the left mouse button:
                self.funcid_button1_start_move_of_selection = self.canvas.tag_bind(
                    "selected", "<Button-1>", lambda event: self._move_selection_start(event, references_of_copies)
                )
            self.copy_button.configure(state="disabled")

    def _store_visible_center_point(self):
        visible_center_point = [
            (self.canvas.canvasx(0) + self.canvas.canvasx(self.canvas.winfo_width())) / 2,
            (self.canvas.canvasy(0) + self.canvas.canvasy(self.canvas.winfo_height())) / 2,
        ]
        self.window.design.store_visible_center_point(
            visible_center_point, push_design_to_stack=True, signal_design_change=False
        )
    ...
    def _view_instance(self):
        file_name = self._search_filename_of_module_in_design_dict(
            self.window.design.get_module_name(), "", self.window.hierarchytree.top_dict
        )
    ...
    def _search_filename_of_module_in_design_dict(self, module_name, last_file_name, design_dict):
        ...
        if design_dict["sub_modules"]:
            for sub_module_dict in design_dict["sub_modules"]:
                file_name = self._search_filename_of_module_in_design_dict(
                    module_name, design_dict["filename"], sub_module_dict
                )
                if file_name != "":
                    return file_name
        return ""
    ...
    def view_all(self):
        """Zooms out to the point, where all elements of the canvas are visible."""
    ...
    def _start_drawing_zoom_rectangle_by_button(self, event):
        ...
        self.funcid_button1_release = self.canvas.bind(
            "<ButtonRelease-1>", lambda event: self._close_zoom_rectangle_by_button(zoom_rectangle_id)
        )
    ...
    def zoom_area(self, zoom_coords, zoom_command):
        ...
        if zoom_coords[0] != zoom_coords[2] and zoom_coords[1] != zoom_coords[3]:
            ...
            overlapping_canvas_ids = self.canvas.find_overlapping(
                zoom_coords[0] - 5, zoom_coords[1] - 5, zoom_coords[0] + 5, zoom_coords[1] + 5
            )
    ...
    def zoom(self, factor, zoom_command, event):
        ...
        if event is None:
            ...
            delta_x = (
                mouse_pointer_x_measured_from_upper_left_corner_of_screen
                - canvas_upper_left_x_measured_from_upper_left_corner_of_screen
            )
            delta_y = (
                mouse_pointer_y_measured_from_upper_left_corner_of_screen
                - canvas_upper_left_y_measured_from_upper_left_corner_of_screen
            )
        ...
        # When zoom is started by a button, then the focus is on the button and not at canvas anymore:
        self.canvas.focus_set()
    ...
    def adjust_scroll_region_at_zoom(self, factor_adapted):
        """Adapt the scroll region at zoom"""
    ...
    def _get_new_font_size(self, factor, zoom_command):
        ...
        if (
            new_font_size_int == self.design.get_font_size()  # The casting into "int" reduced the factor to 1,
            and factor > 1  # but the picture shall increase (factor>1) and it is not a "view all",
            and zoom_command != "view_all"
        ):  # then the font size can be increased (to determine the real zoom-factor).
    ...
    def update_diagram_tab_from(self, new_design, push_design_to_stack):
        """Update the diagram tab from the given design. Called by file reading and by design_data_selector."""
        # Is called only at file-read.
        # Needed before update_diagram_tab, so that the tab gets visible and gets its correct width/height for view_all:
        ...
        self.update_diagram_tab(new_design, push_design_to_stack=False)  # push_design_to_stack will be done by view_all
        self.view_all()  # Not done by update_diagram_tab, because update_diagram_tab is used also by Undo/Redo.

    def update_diagram_tab(self, new_design, push_design_to_stack):
        """Load a new design into the diagram tab. Called by update_diagram_tab_from and by Undo/Redo."""
        references = self._get_references_without_signalnames("all")
        for reference in references:
            reference.delete_item(
                push_design_to_stack=False
            )  # The changes by delete shall not be tracked step by step.
        self.design.update_window_title(
            written=True
        )  # Each delete_item marks the window-title with '*'. But this is wrong in this case and fixed here.
        ...
        self.design.store_visible_center_point(
            new_design["visible_center_point"], signal_design_change=False, push_design_to_stack=False
        )
        wire_ref = None
        dummy = None
        for canvas_id in new_design["canvas_dictionary"]:
            if new_design["canvas_dictionary"][canvas_id][1] == "input":
                interface_input.Input(
                    ...
                    self,
                    dummy,
                    follow_mouse=False,  # push_design_to_stack=False,
        ...
        # Needed to prevent winfo_width/height from being 1 in the following lines (only at the first read from file):
        ...
        if self.window.state != "withdrawn":
            ...
            self.grid_drawer.draw_grid()
        self.canvas.configure(confine=True)
        self.design.add_change_to_stack(
            push_design_to_stack
        )  # push_design_to_stack is only true, when data is read from a file.
        if wire_highlight.WireHighlight.highlight_object is not None and self.window.winfo_viewable() == 1:
    ...
    def _start_drawing_selection_rectangle(self, event):
        ...
        canvas_ids_overlapping = self.canvas.find_overlapping(
            event_x - self.design.get_grid_size() / 10,
            event_y - self.design.get_grid_size() / 10,
            event_x + self.design.get_grid_size() / 10,
            event_y + self.design.get_grid_size() / 10,
        )
        found_item = 0
        for canvas_id_overlapping in canvas_ids_overlapping:
            if "grid_line" not in self.canvas.gettags(canvas_id_overlapping):
                found_item += 1
        if found_item == 0:
            select_rectangle = self.canvas.create_rectangle(event_x, event_y, event_x, event_y, dash=(3, 5))
            self.funcid_motion = self.canvas.bind(
                "<Motion>", lambda event: self._expand_rectangle(event, select_rectangle)
            )
            self.funcid_button1_release = self.canvas.bind(
                "<ButtonRelease-1>", lambda event: self._select_items_in_selection_rectangle(select_rectangle)
            )

    def _unselect_elements(self, references=()):
        if (
            self.funcid_button1_start_move_of_selection is not None
        ):  # Remove the binding of _move_selection_start from all selected elements.
            self.canvas.tag_unbind("selected", "<Button-1>", self.funcid_button1_start_move_of_selection)
            self.funcid_button1_start_move_of_selection = None
    ...
    def _get_references_without_signalnames(self, tag):
        ...
        for selected_canvas_id in selected_canvas_ids:
            ...
            if ref != []:
                # There are Canvas items, which do not have a reference, for example the polygons of an instance.
                ...
                # A canvas item is found by the tag, which has no entry in the canvas dictionary,
                # but belongs to a bigger canvas object.
                pass
        return references_to_selected

    def delete_selection(self):
        """Deletes the selected elements. Called by "Delete" key and by the delete button."""
    ...
    def _select_items_in_selection_rectangle(self, select_rectangle):
        ...
        if references:
            self.canvas.dtag(
                "selected", "selected"
            )  # Remove the selection in order to expand it to not selected parts of the selected elements.
            all_object_tags = []
            for reference in references:
                all_object_tags.append(reference.get_object_tag())
            for object_tag in all_object_tags:
            ...
            for reference in references:
                reference.select_item()  # Highlights all items and deactivates all bindings at the items
            self.funcid_button1_start_move_of_selection = self.canvas.tag_bind(
                "selected", "<Button-1>", lambda event: self._move_selection_start(event, references)
            )
            ...
            if canvas_ids_selected:  # At least one canvas item is enclosed in the selection rectangle.
                ...
                # When the polygon selection is used, then the red color and the "selected" tag are
                # removed by PolygonMove.
    ...
    def get_polygon_move_list(self):
        """Called by symbol_polygon_move"""
        return_list = list(self.polygon_move_list)
        self.polygon_move_list = []
        return return_list

    def _move_selection_start(self, event, references):
        # Because the mouse click after paste (to end the moving of the new elements) shall not start a new selection:
        self.remove_canvas_bindings()
        ...
        if self.funcid_button1_release is None:  # Check is needed, because _move_selection-start may be called twice.
            ...
            # To determine the distance from the grid after the last moving an anchor object is created here, which
            # can be identified by the tag "move_anchor". This anchor object is placed at the grid before movement by
            # using 2 coordinates of one of the canvas widgets, which have to be moved.
            ...
            # For the move_anchor an object is used, which is not used for any other purpose, because
            # the checks for finding overlapping items do not search or check for an "arc":
            self.canvas.create_arc(
                coords_of_anchor_object[0],
                coords_of_anchor_object[1],
                coords_of_anchor_object[0],
                coords_of_anchor_object[1],
                tags=("move_anchor", "selected"),
                width=0,
            )
            self.funcid_motion = self.canvas.bind("<Motion>", self._move_selection_to)
            self.funcid_button1_release = self.canvas.bind(
                "<ButtonRelease-1>", lambda event: self._move_selection_end(event, references, coords_of_anchor_object)
            )
    ...
    def _store_moved_elements(self, references):
        ...
        references[0].store_item(
            push_design_to_stack=True, signal_design_change=True
        )  # Store again, in order to create 1 stack entry

    def _move_selection_to_grid(self, tag_or_id, _, coords_of_anchor_object):
    ...
    def undo(self):
        """Undoes the last change. Called by "Ctrl-z" and by the undo button."""
        ...
        if new_design is not None:
            ...
            # Needed because at update_diagram_tab (see line before) the push_design_to_stack has the value False:
            self.design.update_hierarchy()

    def redo(self):
        """Redoes the last undone change. Called by "Ctrl-y" and by the redo button."""
    ...
    def find_string(self, search_string, replace, new_string):
        """Finds the given string in the diagram and optionally replaces it. Called by "Ctrl-f" and by find button."""
        number_of_hits = 0
        all_canvas_ids = self.canvas.find_all()
        for canvas_id in all_canvas_ids:
            if (
                self.canvas.type(canvas_id) == "text"
            ):  # block, signal_name (not the declaration), instance_name, generic_map, generate-condition
                if replace:
                    all_tags = self.canvas.gettags(canvas_id)
                    # No replacement in entity-name or symbol-port-names:
                    if (
                        "block-text" in all_tags
                        or "signal-name" in all_tags
                        or "instance-name" in all_tags
                        or "generate-frame" in all_tags
                        or "generic-map" in all_tags
                    ):
    ...
    def _store_new_architecture_name(self, ask_architecture_window):
        ...
        if new_architecture_name not in self.architecture_list:
            ...
            self.design.create_new_and_empty_schematic(
                self.architecture_name
            )  # Design-Data of self.architecture_name is stored first.
    ...
    def _delete_architecture_after_idle(self):
        if len(self.architecture_list) > 1:
            delete_architecture = messagebox.askyesno(
                "Delete architecture",
                "Are you sure you want to delete the architecture "
                + self.architecture_name
                + "? All data will be removed from the database.",
            )
    ...
    def _change_architecture_name(self, ask_architecture_window):
        ...
        if new_architecture_name not in self.architecture_list:
            ...
            messagebox.showerror(
                "Renaming-Error", "New architecture name " + new_architecture_name + " already exists."
            )
    ...
    def clear_canvas_for_new_schematic(self):
        """Called by design.create_new_and_empty_schematic()"""
        all_canvas_ids = self.canvas.find_all()
        for canvas_id in all_canvas_ids:
    ...
    def _open_entry_window(self):
        ...
        architecture_entry = ttk.Entry(
            ask_architecture_window, width=80, justify="left", textvariable=self.architecture_name_stringvar
        )
    ...
    def highlight_item(self, hdl_item_type, object_identifier, number_of_line, use_external_editor):
        ...
        if hdl_item_type in [
            "port_declaration",
            "signal_declaration",
        ]:  # The port_declaration does not contain "in", "out", "inout" anymore.
            ...
            edit_text.EditText(
                ...
                self.window,
                self,
                symbol_reference.symbol_definition["generic_block"]["canvas_id"],
                symbol_reference,
                number_of_line,
            )
    ...
    def update_all_instances(self):
        """Updates all instances by updating the symbol of each instance."""
        for canvas_id in self.canvas.find_all():
            if (
                canvas_id in self.design.get_canvas_ids_of_elements()
