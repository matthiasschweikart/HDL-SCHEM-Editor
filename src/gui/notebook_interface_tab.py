...
class NotebookInterfaceTab:
    ...
    def __init__(self, schematic_window, notebook):
        ...
        self.interface_packages_scroll = ttk.Scrollbar(
            self.packages_frame, orient=tk.VERTICAL, cursor="arrow", command=self.interface_packages_text.yview
        )
        ...
        self.interface_packages_scroll.grid(
        ...
        self.interface_generics_scroll = ttk.Scrollbar(
            self.generics_frame, orient=tk.VERTICAL, cursor="arrow", command=self.interface_generics_text.yview
        )
        ...
        self.interface_generics_scroll.grid(
            row=1, column=1, sticky=(tk.W, tk.E, tk.S, tk.N)
        )  # "W,E" nötig, damit Text tatsächlich breiter wird
    ...
    def adjust_sash_positions(self) -> None:
        ...
        if self._abort_after_storing_new_height():
            return
        if self.window.design.get_language() == "VHDL":
            text_list = [self.interface_packages_text, self.interface_generics_text]
        else:
            text_list = [self.interface_generics_text]
