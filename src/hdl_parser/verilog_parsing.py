...
class VerilogParser:
    ...
    def __init__(self, verilog, region="module", parse_big_files=False):
        ...
        self.parse_result[
            "keyword_positions"
        ] = []  # There is no entry with the key "keyword" as such a list is useless.
    ...
    def _get_next_words(self, reg_ex_list_for_splitting_into_words):
        ...
        return [word1, start_index_of_word_before_search_string, end_index_of_word_before_search_string], [
            word2,
            start_index_of_search_string_match,
            end_index_of_search_string_match,
        ]
    ...
    def _analyze(self, word_list):
        ...
        for word in word_list:
            ...
            if word[0].startswith("//"):
                ...
                if word[0] == "=":
                    ...
                    self.parse_result["generics_interface_ranges"].append(
                        ""
                    )  # Default value, as a parameter must not have a range.
                ...
                if word[0] not in ["", " ", "\n", "\r", "\t"]:  # Jump over whitespaces
                    self.parse_result["port_interface_direction"].append(
                        word[0]
                    )  # word[0] is "input", "output" or "inout".
                    ...
                    self.parse_result["port_interface_types_positions"].append(
                        [0, 0]
                    )  # Default value, as a type must not exist.
        ...
        parameter_definition = re.sub(
            r"(?m)^\s*", "", parameter_definition
        )  # remove leading blanks, multiline flag is set
